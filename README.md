...
- New tasks are assigned through `CLICKUP_ASSIGNEE_ID` (Isabella in production), include ClickUp's `notify_all` flag so the API-token user still gets the normal ClickUp inbox/email notification when the token belongs to Isabella, and add Marketing group members as watchers when the ClickUp group lookup is configured; submissions from `isabella@richardsandoval.com` route directly to `CLICKUP_POST_APPROVAL_STATUS` (`To Do` by default), assign the resolved Marketing users, and update Menu Manager to `sent_to_marketing` instead of leaving the row in Isabella's review queue, even when `CLICKUP_CORRECTIONS_STATUS` is a different review-complete trigger such as `approved`
...
- The approval editor keeps imported redline token boundaries while reviewers type, so adjacent DOCX replacement spans do not fuse into synthetic words or duplicate old deletion text when an accepted word is removed.
