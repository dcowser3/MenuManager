...
The `/learning` dashboard separates auto-scanned detected patterns from active Pre-AI rules: detected patterns are candidate evidence for reviewer annotation, while only accepted safe exact replacement rules in the Active Pre-AI section can change submitted menu text. Pending Rules shows only unconsumed corrections still awaiting direct review; corrections already consumed by an approved prompt proposal are represented by the generated accepted rules instead. The improvement loop accepts only human-attributed corrections with a reviewer explanation; auto-detected patterns, blank explanations, and menu-only content updates do not enter proposal generation. Reviewers can mark an outside-discussion edit as **Menu/content update only** or scope a mixed dish-line correction to the exact From/To replacement its explanation covers. The proposal page shows that explanation beside each correction. The loop emails when a proposal is ready and sends a reminder only when both the explanation queue and the live review baseline are unchanged. New explanations—or a deployed prompt, model, code-rule, or accepted-rule change—automatically replace a stale pending proposal. Full-line reviewer corrections are replay-verified against the current pipeline; corrections replay proves are already fixed are excluded from the next proposal and stay retired instead of recycling after an unrelated proposal rejection, while compound rows can be marked partially correct so only their unresolved portion is proposed again. Proposal metadata separates entered, retired, and actionable counts from carried/new lineage. Prompt rewrites with confirmed regressions, failed/skipped evals, no measured effect, unresolved replay misses, or unscored motivating submissions cannot be approved. Proposed deterministic rules are activated transiently during evaluation, and the proposal records full-suite pre-AI/post-AI evidence plus targeted replay against the motivating unresolved correction text; a rules-only proposal cannot pass or be approved unless every rule fires in at least one of those paths with no confirmed regressions. Regressed prompt-plus-rules proposals identify whether each confirmed failure comes from the prompt, replacement rules, both, or their interaction; when every regression is prompt-only, an uncapped full-suite rules-only run can automatically salvage the safe rules without carrying the bad rewrite into another cycle. The page displays confirmed back-to-back regression deltas when available. Detected patterns and the accepted-rule audit log show the implementation lane and what the code does next time, or why the note remains guidance only.
