...
- Basic AI Check treats standalone selection instructions such as `choose one`, `choice of one`, `select two`, and `pick your entree` as menu instructions, not dish entries, so they do not create critical Incomplete Dish Name blockers.
...
- The Step 2 editor layout keeps the left menu editor and first visible menu row aligned with the first right-side review box across brand-new submissions, database-baseline modifications, and uploaded/redlined DOCX modification flows.
- Extracted DOCX rich HTML trims leading empty paragraphs before the browser editor renders, preventing a stray blank editable row above the first menu line.
...
- Production public-form failure events also email `FORM_ATTEMPT_ALERT_EMAIL`, defaulting to `dcowser@richardsandoval.com`; dashboard alert email sends via Microsoft Graph `sendMail` (HTTPS) when Graph credentials and `GRAPH_MAILBOX_ADDRESS` are configured, with SMTP as fallback (Lightsail blocks outbound port 25, so the SMTP relay path cannot connect from production)
...
- The submission form footer, blocking/red form errors, and the critical-error banner now offer a one-click **"Report this problem"** button that saves full incident details under `tmp/error-reports/<incidentId>/`, logs `form_attempt_logs`, and emails support (`FORM_ATTEMPT_ALERT_EMAIL`) a lightweight incident notice with the incident id instead of bulky attachments; oversized reports use a route-specific 15 MB JSON cap and byte-budgeted client compaction; screenshot capture normalizes Safari CSS color functions; successful final submits clear stale red error report actions; production can email a follow-up AI triage proposal when `OPENAI_API_KEY` is configured; `PUBLIC_FORM_SUPPORT_EMAIL` (default `dcowser@richardsandoval.com`) remains the manual email fallback. See [docs/design-docs/user-error-reports.md](docs/design-docs/user-error-reports.md).
...
- Notification system
- Approved dishes are extracted automatically when the ClickUp-reviewed DOCX reaches the `To Do` handoff or browser approval finalization completes
...
- New tasks are assigned through `CLICKUP_ASSIGNEE_ID` (Isabella in production), include ClickUp's `notify_all` flag, and add an assigned reviewer comment with `notify_all: true` so Isabella gets a ClickUp notification even when the task is created by Isabella's API token; Marketing group members are added as watchers when the ClickUp group lookup is configured; submissions from `isabella@richardsandoval.com` route directly to `CLICKUP_POST_APPROVAL_STATUS` (`To Do` by default), assign the resolved Marketing users, and update Menu Manager to `sent_to_marketing` instead of leaving the row in Isabella's review queue
- ClickUp status `Approved` is passive/manual for Menu Manager webhooks. Moving a task to `Approved` does not download a DOCX, finalize the submission, move the task back to `To Do`, or change assignees, even if `CLICKUP_CORRECTIONS_STATUS` or `CLICKUP_CORRECTIONS_STATUSES` accidentally includes `approved`.
- ClickUp review-complete webhooks skip submissions already marked as Isabella direct handoffs, so a later `To Do` webhook will not download a DOCX, move the task, or reassign Marketing
...
- When the approved DOCX is processed from a configured review-complete status, the task is assigned to the resolved Marketing users and the configured initial reviewer is removed when applicable; the task is then moved to `To Do`, or the status update is skipped if it is already there
...
- Isabella can open `/reviews` directly to see every pending human-review or manual-review submission; the queue is intentionally unlisted from the main welcome dashboard. Submissions whose linked ClickUp task was deleted can be marked `deleted` so they are preserved in the database without appearing in the queue.
...
- Isabella can edit approved text in a left-side rich browser editor while a right-side panel shows the live tracked-change preview recreated from the original DOCX text and the current accepted text.
- For submitted DOCX files that already contain redlines, the left editor uses the clean accepted text with deleted runs removed, accepted inserted runs unwrapped, and DOCX inline formatting such as bold dish names preserved; the preview rebuilds the redlines by comparing the reconstructed original text with the current left-editor text.
...
- The approval editor derives two canonical text sides from imported DOCX redlines: original text keeps deleted/unmarked runs, fixed text keeps inserted/unmarked runs. The preview then renders `original -> current editor text` on every update, so later edits cannot inject old deletion runs back into the accepted text.
- The live approval preview keeps adjacent replacement spans readable after new edits, so corrections like `jalapeno` → `jalapeño`, `neapolitan` → `Neapolitan`, or adjacent DOCX replacements do not fuse into synthetic words when another word is removed.
- The live approval preview keeps whole inserted/deleted rows separate after unrelated nearby edits, so a top-line wording change does not duplicate or move redlined text on the next menu item.
- Divider/header structure is matched as literal text when a row has no word-like tokens, so unchanged dashed separators do not become wildcard anchors for edited dish rows; if the divider text itself changes, it is still highlighted like any other text.
...
- The approval preview worker receives the canonical original text instead of resolver/replay state, keeping worker and main-thread preview renders on the same `original -> current` contract.
...
- Uploaded unapproved/redlined DOCX modifications do not trigger the database-baseline replacement dialog after extraction, so preserved chef highlights/redlines remain the submission source even when the property already has an approved database baseline.
...
- The approval editor renderer treats whitespace as a display boundary rather than a standalone diff anchor inside changed runs, so adjacent DOCX replacement spans do not fuse into synthetic words or duplicate old deletion text when an accepted word is removed.
...
- Browser approval finalization uses `CLICKUP_APPROVAL_FINALIZE_TIMEOUT_MS` (default `60000`) so uploading the approved DOCX, assigning Marketing, and moving the ClickUp task do not inherit the shorter internal-service timeout.
...
- Approved-dish rows show source metadata from the originating submission, including filename/project, source type, ClickUp task when available, IDs, and source-line context when available. New extraction runs prefer the parser-captured source line and line number before falling back to fuzzy source matching.
...
- Wrapped dish rows are joined before parsing so continuation ingredients such as `gochujang` stay in the description instead of becoming standalone dish names; service hours, weekday labels, per-guest/package labels, course labels, event instructions, attribution lines, short beverage headings such as `RED`/`GIN`, grill/service labels, oatmeal topping continuations, fused pricing grids such as `À La Carte PricingAntojitos`, and modifier rows such as `add chicken` are skipped as non-dish metadata. Beverage section headers such as `Pick Me Up`, `Cocteles`, `Zero Proof`, `Espumoso`, `Blanco`, `Rosado`, `Rojo`, `Cerveza`, `Flights`, and `Vino by the Bottle` become categories instead of dish names, and leader-dot names such as `Acqua Panna 1 liter........ 8` are stored without the layout dots. Beverage sections can reuse bare price-only rows such as `18` for the following drink rows, and `name price` rows can consume a following ingredient line as their description. When a terse dish name is enriched from its section, the inferred word is added in parentheses, e.g. `Kale (Salad)`.
...
- The shared quality analyzer flags pricing grids, category/description contamination, beverage headings stored as names, layout leaders left in names, beverage name/description swaps, instruction text, missing provenance, low-information plausible rows, and exact repeated rows. Only targeted risky DB extraction rows are sent to ai-review in 100-row batches for an advisory `dish` / `not_dish` / `uncertain` check; legitimate bare beverage price-list rows are not reviewed just because they lack descriptions. Only high-confidence `not_dish` responses are excluded from future writes.
