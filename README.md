...
- New tasks are assigned through `CLICKUP_ASSIGNEE_ID` (Isabella in production), include ClickUp's `notify_all` flag, and add an assigned reviewer comment with `notify_all: true` so Isabella gets a ClickUp notification even when the task is created by Isabella's API token; Marketing group members are added as watchers when the ClickUp group lookup is configured; submissions from `isabella@richardsandoval.com` route directly to `CLICKUP_POST_APPROVAL_STATUS` (`To Do` by default), assign the resolved Marketing users, and update Menu Manager to `sent_to_marketing` instead of leaving the row in Isabella's review queue, even when `CLICKUP_CORRECTIONS_STATUS` is a different review-complete trigger such as `approved`
