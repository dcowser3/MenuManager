...
describe('graph mail configuration', () => {
    test('is configured only when every credential and a sendable mailbox exist', () => {
        expect(isGraphMailConfigured(GRAPH_ENV)).toBe(true);
        expect(isGraphMailConfigured({ ...GRAPH_ENV, GRAPH_CLIENT_SECRET: '' })).toBe(false);
        expect(isGraphMailConfigured({ ...GRAPH_ENV, GRAPH_MAILBOX_ADDRESS: '' })).toBe(false);
        expect(isGraphMailConfigured({})).toBe(false);
    });

    test('falls back to GRAPH_USER_EMAIL for the mailbox address', () => {
        expect(getGraphMailboxAddress({ GRAPH_USER_EMAIL: 'fallback@example.com' })).toBe('fallback@example.com');
        expect(getGraphMailboxAddress({ ...GRAPH_ENV, GRAPH_USER_EMAIL: 'ignored@example.com' }))
...
describe('graph sendMail request', () => {
    ...
    test('omits attachment and cc keys when there are none', () => {
        const request = buildGraphSendMailRequest({ to: 'a@example.com', subject: 's', html: '<p>h</p>' });
...
describe('sendGraphMail', () => {
    test('posts to the mailbox sendMail endpoint with a bearer token', async () => {
        const fetchImpl = jest.fn(async () => ({ ok: true, status: 202, text: async () => '' })) as any;
