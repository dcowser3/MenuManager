...
exports.generateRedlinedDocx = generateRedlinedDocx;
const docx_1 = require("docx");
const fs_1 = require("fs");
async function generateRedlinedDocx(originalText, aiCorrections, outputPath) {
    ...
    for (const change of changes) {
        if (change.content.includes('\n\n') || change.content.includes('---')) {
            ...
            if (currentParagraphRuns.length > 0) {
                paragraphs.push(new docx_1.Paragraph({ children: currentParagraphRuns }));
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (change.type === 'delete') {
                currentParagraphRuns.push(new docx_1.TextRun({
                    text: line,
                    strike: true,
            ...
            if (i < lines.length - 1 && lines[i + 1].trim()) {
                currentParagraphRuns.push(new docx_1.TextRun({ text: ' ' }));
            }
        }
    }
...
function parseAICorrections(aiText) {
    const changes = [];
