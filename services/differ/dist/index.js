...
async function initDiffer() {
    try {
        ...
        try {
            await fs_1.promises.access(TRAINING_DATA_FILE);
        }
        catch {
            await fs_1.promises.writeFile(TRAINING_DATA_FILE, '');
        }
    }
    catch (error) {
...
function analyzeDocuments(aiDraft, final) {
    ...
    const summary = {
        ...
        word_count_diff: Math.abs(final.split(/\s+/).length - aiDraft.split(/\s+/).length),
