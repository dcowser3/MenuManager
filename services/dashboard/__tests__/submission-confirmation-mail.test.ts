...
describe('buildSubmissionReceiptHtml', () => {
    ...
    test('approver dispute link appears only when includeDisputeLink is set (approver copy)', () => {
        ...
        const submitterCopy = buildSubmissionReceiptHtml(withToken, false, 'https://dash.example.com');
        const approverCopy = buildSubmissionReceiptHtml(withToken, false, 'https://dash.example.com', { includeDisputeLink: true });
    ...
    test('no dispute link when there is no token even if requested', () => {
        const html = buildSubmissionReceiptHtml(input({ approverDisputeToken: '' }), false, 'https://dash.example.com', { includeDisputeLink: true });
        expect(html).not.toContain('/approval-dispute/');
    });

    test('buildApproverDisputeUrl builds a link or empty string', () => {
        expect(buildApproverDisputeUrl('https://dash.example.com/', 'abc')).toBe('https://dash.example.com/approval-dispute/abc');
        expect(buildApproverDisputeUrl('https://dash.example.com', '')).toBe('');
    });
});
