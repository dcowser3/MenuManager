import {
    ...
    buildExpectationEnvelopeFromTrustedProposal,
...
test('proposal producer creates only explicit human-evidence supersession envelopes', () => {
    ...
    const envelope = buildExpectationEnvelopeFromTrustedProposal({ proposalId: 'cycle-1', proposedRules: [rule] });
    ...
    expect(buildExpectationEnvelopeFromTrustedProposal({ proposalId: 'cycle-1', proposedRules: [{ ...rule, change_type: 'terminology' }] })).toBeNull();
    expect(buildExpectationEnvelopeFromTrustedProposal({ proposalId: 'cycle-1', proposedRules: [{ ...rule, expectation_activation: { ...rule.expectation_activation, sourceRevision: null } }] })).toBeNull();
});
