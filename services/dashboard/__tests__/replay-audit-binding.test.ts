import {
    bindReplayAudit,
    normalizeReplayAudit,
} from '../lib/replay-audit-binding';
import {
...
describe('trusted same-attempt replay-audit binding', () => {
    test('binds exactly one completed/full audit for the exact attempt', () => {
        const result = bindReplayAudit([row()], attempt);
    ...
    test.each([
        ...
        expect(bindReplayAudit(rows, submissionAttempt)).toMatchObject({ eligible: false, reason });
    });

    test('malformed input and missing attempt fail closed without choosing recency', () => {
        expect(bindReplayAudit({ not: 'rows' }, attempt)).toMatchObject({ eligible: false, reason: 'malformed_audit' });
        expect(bindReplayAudit([row()], '')).toMatchObject({ eligible: false, reason: 'missing_submission_attempt' });
        expect(normalizeReplayAudit({ ...row(), final_result: { corrected_menu: original } })).toBeNull();
    });

    test('same-attempt proof uses deterministic replay only and records zero model calls', () => {
        const binding = bindReplayAudit([row()], attempt);
        if (!binding.eligible) throw new Error('fixture should bind');
        let providerCalls = 0;
        const replay = replayOriginalResponseDeterministically(binding.audit, {
