...
import {
    buildApprovedDishVocabularyTerms,
    deriveBrandFromProperty,
    getApprovedDishBrandDetail,
    listApprovedDishBrands,
    loadApprovedReviewVocabularyTerms,
    mergeApprovedVocabularyTerms,
    slugifyApprovedDishBrand,
...
describe('approved dish browse helpers', () => {
    ...
    test('derives stable brand names and slugs from canonical properties', () => {
        expect(deriveBrandFromProperty('Toro Toro - InterContinental - Miami')).toBe('Toro Toro');
        expect(deriveBrandFromProperty('Toro Del Mar - Athens')).toBe('Toro Del Mar');
        expect(slugifyApprovedDishBrand('tán')).toBe('tan');
    });

    test('aggregates frequent approved culinary words without losing accents', () => {
        const terms = buildApprovedDishVocabularyTerms([
    ...
    test('merges approved dish and full-menu vocabulary counts', () => {
        expect(mergeApprovedVocabularyTerms(
            [{ term: 'Tamarind', count: 2 }, { term: 'fuego', count: 1 }],
            [{ term: 'tamarind', count: 3 }, { term: 'brûlée', count: 4 }]
        )).toEqual([
    ...
    test('combines active dish rows with full approved menu text', async () => {
        const terms = await loadApprovedReviewVocabularyTerms(repoRoot);
        expect(terms).toEqual(expect.arrayContaining([
            { term: 'lomo', count: 2 },
    ...
    test('lists brand summaries with location counts from approved dishes', async () => {
        const summaries = await listApprovedDishBrands(repoRoot);
    ...
    test('returns a brand detail grouped by location with search and location filters', async () => {
        const detail = await getApprovedDishBrandDetail(repoRoot, 'toro-toro', {
            query: 'dulce',
