...
describe('assessCorrectedMenuStructure', () => {
    ...
    it('rejects a corrected menu that drops most menu lines', () => {
        const original = Array.from({ length: 18 }, (_, i) =>
            `Dish ${i + 1}, roasted ingredient ${i + 1}, citrus salsa, herb garnish, crispy tortilla G,V ${18 + i}`
        ).join('\n');
        const corrected = [
    ...
    it('rejects a condensed wall-of-text response that omits most submitted words', () => {
        ...
        const corrected = Array.from({ length: 8 }, (_, i) =>
            `Dish ${i + 1} description with avocado corn poblano chili sesame mango coconut allergen code G,V price ${20 + i}`
        ).join('\n');
    ...
    it('does not block very short menus where coverage metrics are noisy', () => {
        const result = assessCorrectedMenuStructure(
            'Guacamole, avocado, tortilla chips V',
            'Guacamole, avocado, tortilla chips V'
        );
