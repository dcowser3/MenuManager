...
test('durable claim crash before summary converges to blocked zero-dispatch recovery', async () => {
    ...
    const store = { recordCodeVerification: async (_client, original, patch) => { state.writes += 1; state.proposal = { ...original, eval_summary: { ...original.eval_summary, code_candidate: patch.code_candidate } }; } };
