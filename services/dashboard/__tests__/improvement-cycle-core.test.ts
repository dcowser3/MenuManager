...
describe('computeReviewBaselineFingerprint', () => {
    ...
    test('changes for behavior inputs but ignores volatile rule metadata and entry order', () => {
        const base = computeReviewBaselineFingerprint({
            prompt: 'prompt A',
            reviewModel: 'gpt-5.6-luna',
            manifestEntries: [entry, { id: 'z-rule', description: 'Z' }],
        });
        const reordered = computeReviewBaselineFingerprint({
            prompt: 'prompt A',
            reviewModel: 'gpt-5.6-luna',
            manifestEntries: [
        ...
        expect(computeReviewBaselineFingerprint({
            prompt: 'prompt B',
            reviewModel: 'gpt-5.6-luna',
            manifestEntries: [entry, { id: 'z-rule', description: 'Z' }],
        })).not.toBe(base);
...
describe('needsDistinctCycleId', () => {
    test('allocates a unique id for same-day automatic supersedes as well as forced runs', () => {
        expect(needsDistinctCycleId({ existingProposal: { status: 'pending' }, supersedeEligible: true })).toBe(true);
        expect(needsDistinctCycleId({ existingProposal: { status: 'pending' }, force: true })).toBe(true);
        expect(needsDistinctCycleId({ existingProposal: { status: 'pending' } })).toBe(false);
        expect(needsDistinctCycleId({ existingProposal: null, supersedeEligible: true })).toBe(false);
    });
});
...
describe('correctionsEligibleForImprovement', () => {
    ...
    test('learning_intent takes precedence and menu-content updates never enter proposal input', () => {
        ...
        expect(correctionsEligibleForImprovement([row])).toEqual([]);
        const learned = { ...row, learning_intent: 'missed_review_correction' };
        expect(correctionsEligibleForImprovement([learned])).toEqual([learned]);
    });
});
...
describe('eval summary + status', () => {
    ...
    test('attributes combined-candidate regressions to prompt, rules, both, or interaction', () => {
        ...
        const armReport = (entries: Array<{ case_id: string; delta: number }>) => ({
            baselineComparison: { regressions: entries.map((entry) => ({ ...entry, confirmed_delta: entry.delta })) },
        });
        const attribution = buildRegressionAttribution(regressions, {
            promptChanged: true,
            ruleCount: 3,
...
describe('buildPendingProposalReminderEmail', () => {
    test('builds a reminder for the proposal blocking the next cycle', () => {
        const message = buildPendingProposalReminderEmail({
            proposal: {
                id: 'prop-1',
                cycle_id: '2026-06-26',
...
describe('decideReplayStatus (Follow-up 2)', () => {
    ...
    test('recognizes a full-line correction when replay applies the atomic change', () => {
        ...
        expect(fullLineCorrectionApplied(original, corrected, replay)).toBe(true);
        expect(decideReplayStatus(original, corrected, replay, [sig('jalapeños', 'jalapeño')])).toBe('now_correct');
    });
    ...
    test('allows unrelated same-line cleanup while requiring the complete reviewer delta', () => {
        ...
        expect(fullLineCorrectionApplied(
            original,
            corrected,
            'Dish, yellow chili aioli, pickle D, S 75'
        )).toBe(true);
        expect(fullLineCorrectionApplied(
            original,
            corrected,
            'Dish, yellow chili aioli, cucumber pickle D, S 75'
        )).toBe(false);
    });
...
describe('buildImprovementLlmPayload (F0 extraction)', () => {
    ...
    test('gpt-5 family routes down the reasoning path (no temperature — gpt-5 rejects it)', () => {
        for (const model of ['gpt-5', 'gpt-5.1', 'gpt-5-mini', 'gpt-5.4-pro-2026-03-05']) {
            const p = buildImprovementLlmPayload(model, 's', 'u');
            expect(p).not.toHaveProperty('temperature');
            expect(p).not.toHaveProperty('max_tokens');
            expect(p.max_completion_tokens).toBe(32000);
        }
...
describe('isReasoningModel', () => {
    ...
    test('does not match gpt-4o family or empty', () => {
        for (const m of ['gpt-4o', 'gpt-4o-mini', 'gpt-4.1', 'gpt-4-turbo', '', undefined as unknown as string]) {
...
describe('isRequestTooLarge429 (fail-fast guard)', () => {
    test('matches the request-too-large TPM body', () => {
        ...
        expect(isRequestTooLarge429(body)).toBe(true);
    });

    test('does not match a normal transient 429 or empty body', () => {
        expect(isRequestTooLarge429('{"error":{"message":"Rate limit reached for o3: try again in 12s","type":"requests"}}')).toBe(false);
        expect(isRequestTooLarge429('')).toBe(false);
    });
});
...
describe('C3 validateCorrectionRouting', () => {
    ...
    test('routing carries the human guidance text for freeform corrections', () => {
        const out = validateCorrectionRouting(
            [{ correction_id: 'g', lane: 'prompt', target: 'sec', note: 'ok' }],
            { sourceCorrections: [{ id: 'g', original_text: null, corrected_text: null, rule: 'LAURENT-PERRIER is a proper noun and must be hyphenated.' }] },
        );
    ...
    test('a still_missed correction covered by a narrower synthesized rule does NOT trip unresolved', () => {
        const out = validateImprovementLlmOutput(
            ...
            {
                currentPrompt: 'cur',
                replayEvidence: [{ correction_id: 's1', original_text: 'Esquites sweetyellow corn', corrected_text: 'Esquites sweet yellow corn', status: 'still_missed' }] as any,
