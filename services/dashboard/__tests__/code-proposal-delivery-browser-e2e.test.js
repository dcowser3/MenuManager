...
const IMAGE = 'sha256:a064dc4f63d6782682254cdf3a8546bed06d50007ce3a530a718d5b67c97b41d';
...
test('opt-in causal browser delivery proof and identical-source negative', async () => {
    ...
    const baselineRoot = path.join(attemptRoot, 'baseline'); const candidateRoot = path.join(attemptRoot, 'candidate'); const bundleRoot = path.join(attemptRoot, 'test-bundle'); const outputRoot = path.join(attemptRoot, 'output');
    [baselineRoot, candidateRoot, bundleRoot, outputRoot].forEach((dir) => fs.mkdirSync(dir, { recursive: true, mode: 0o700 }));
    copyTree(baselineRoot, true); copyTree(candidateRoot, false); fs.writeFileSync(path.join(bundleRoot, 'manifest.json'), '{\n  "schema_version": 1,\n  "files": [],\n  "candidate_test_files": []\n}\n', { mode: 0o600 });
    ...
    const result = await runDockerInvocation(buildDockerInvocation({ attemptRoot, attemptId: 'e2e', phase: 'delivery', arm: 'paired', seed: 1, runId: 'e2e:1', imageId: IMAGE, runtimeId: FIXED_RUNTIME_ID, plan, baselineRoot, candidateRoot, testBundleRoot: bundleRoot, outputRoot, request: { phase: 'delivery', arm: 'paired', seed: 1, run_id: 'e2e:1', delivery_fixture: fixture, plan } }), { timeoutMs: 120000, inspectImage: () => IMAGE });
    expect(result.status).toBe('ok'); expect(result.driver).toBe('form-submit-v1'); expect(result.driver_sha256).toBe(workerHash); expect(result.source_manifest_sha256).toBe(sourceManifest); expect(result.baseline_submitted_text).not.toContain('TARGET'); expect(result.baseline_submitted_html_text).not.toContain('TARGET'); expect(result.candidate_submitted_text).toContain('TARGET'); expect(result.candidate_submitted_html_text).toContain('TARGET');
    const negativeRoot = path.join(attemptRoot, 'same'); fs.mkdirSync(negativeRoot, { mode: 0o700 });
    await expect(runDockerInvocation(buildDockerInvocation({ attemptRoot, attemptId: 'same', phase: 'delivery', arm: 'paired', seed: 2, runId: 'same:2', imageId: IMAGE, runtimeId: FIXED_RUNTIME_ID, plan, baselineRoot: candidateRoot, candidateRoot, testBundleRoot: bundleRoot, outputRoot, request: { phase: 'delivery', arm: 'paired', seed: 2, run_id: 'same:2', delivery_fixture: fixture, plan } }), { timeoutMs: 120000 })).resolves.toMatchObject({ status: 'ok', candidate_submitted_text: expect.stringContaining('TARGET') });
});
