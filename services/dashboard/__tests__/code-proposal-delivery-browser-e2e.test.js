...
test('opt-in causal browser delivery proof and identical-source negative', async () => {
    ...
    const same = await runDockerInvocation(buildDockerInvocation({ attemptRoot, attemptId: 'same', phase: 'delivery', arm: 'paired', seed: 2, runId: 'same:2', imageId: IMAGE, runtimeId: FIXED_RUNTIME_ID, plan: samePlan, baselineRoot: candidateRoot, candidateRoot, testBundleRoot: bundleRoot, outputRoot, request: { phase: 'delivery', arm: 'paired', seed: 2, run_id: 'same:2', delivery_fixture: fixture, plan: samePlan } }), { timeoutMs: 120000 });
    expect(same.baseline_submitted_text).toContain('TARGET'); expect(same.candidate_submitted_text).toContain('TARGET'); expect(same.baseline_submitted_html_text).toContain('TARGET'); expect(same.candidate_submitted_html_text).toContain('TARGET'); expect(same.baseline_submitted_text).toBe(same.candidate_submitted_text);
