...
export interface ExpectationEnvelope {
    ...
    policyChangeApprovals?: Array<{ priorId: string; successorId: string; caseId: string; sourceRevisionId: string; status: 'approved'; reviewer: string; approvedAt: string }>;
}
...
export function validateApprovedExpectationAuthority(envelope: ExpectationEnvelope) {
...
export function deriveProposalBoundEnvelope(proposedRules: any[], authority: ExpectationEnvelope, proposalId: string) {
    ...
    if (matches.some((entry) => entry.expectations.length !== 1)) return { rules: proposedRules, envelope: null };
    const candidateToRules = new Map<string, number>();
    for (const entry of matches) { const id = entry.expectations[0].id; if (candidateToRules.has(id)) return { rules: proposedRules, envelope: null }; candidateToRules.set(id, entry.index); }
    ...
    const rules = proposedRules.map((rule, index) => {
        ...
        if (!match || successorIds.has(match.expectations[0].id)) return rule;
