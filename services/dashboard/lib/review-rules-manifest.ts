...
export type ManifestRuleEntry = {
    ...
    data?: unknown;
    source: 'code_data' | 'code_metadata' | 'db_accepted_rule';
};

export type ReviewRulesManifest = {
    entries: ManifestRuleEntry[];
};
...
const FUNCTIONAL_ENTRIES: ManifestRuleEntry[] = [
    {
    ...
    {
        id: 'pre-ai/curated-canonical-food-spelling',
        layer: 'pre_ai_deterministic',
        category: 'spelling',
        title: 'Bounded canonical food-word correction',
        description: 'Reviewer-confirmed food words are matched with bounded Damerau edit distance, including adjacent transpositions. Auto-correction requires a unique match with the same first and last letters, limiting collisions with valid culinary, brand, and foreign-language terms.',
        examples: [
            { before: 'FUGEO MAYO', after: 'FUEGO AIOLI' },
            { before: 'tamrind glaze', after: 'tamarind glaze' },
        ],
        implementation: { file: PRE_AI_FILE, exportName: 'normalizeCuratedFoodSpellingsOnLine' },
        source: 'code_metadata',
    },
    {
    ...
    },
    {
        ...
        title: 'Cooked shrimp ceviche raw-marker exception',
        description: 'Shrimp or prawn ceviche is treated as cooked under the approved house rule, so a raw marker is removed or withheld unless the line explicitly says raw/uncooked/undercooked or contains another independently raw preparation.',
        examples: [{ before: 'Shrimp Ceviche*, lime, avocado C 24', after: 'Shrimp Ceviche, lime, avocado C 24' }],
        implementation: { file: PRE_AI_FILE, exportName: 'normalizeShrimpCevicheRawMarkerOnLine' },
        source: 'code_metadata',
    },
    {
    ...
    },
    {
        ...
        title: 'Accepted reviewer correction rules (bounded replacements)',
    ...
    },
    {
        ...
        layer: 'post_ai_guard',
        category: 'spelling',
        title: 'Unresolved canonical spelling finding guarantee',
        description: 'The model receives contextual near misses first. If it leaves a unique non-ambiguous reviewer/corpus match unchanged and does not mention it, the pipeline adds a medium-confidence, normal-severity spelling suggestion so the possible typo cannot silently pass. Ambiguous matches never synthesize a correction.',
        implementation: { file: 'services/dashboard/lib/canonical-vocabulary.ts', exportName: 'ensureCanonicalSpellingSuggestions' },
        source: 'code_metadata',
    },
    {
        id: 'post-ai/protected-terms',
        layer: 'post_ai_guard',
        category: 'protected_terms',
        title: 'Protected culinary terms',
        description: 'Restores protected phrases that were present in the submitted menu if the AI rewrites them in its corrected-menu output. This guard is anchored to the original line and runs before the remaining post-AI guards.',
        examples: [
...
export function buildReviewRulesManifest(opts: { acceptedCorrectionRules?: AcceptedCorrectionRule[] } = {}): ReviewRulesManifest {
    ...
    for (const [sectionId, meta] of Object.entries(QA_PROMPT_SECTIONS) as Array<[QaPromptSectionId, { description: string; appliesWhen: string }]>) {
        entries.push({
            id: `prompt/${sectionId}`,
    ...
    for (const type of FORCED_CRITICAL_EXACT_TYPES) {
        entries.push({
    ...
    for (const type of FORCED_CRITICAL_NORMALIZED_TYPES) {
        entries.push({
    ...
    for (const rule of opts.acceptedCorrectionRules || []) {
        entries.push({
            ...
            title: rule.original_text && rule.corrected_text
                ...
                : `${(rule as any).rule || 'manual guidance'}`.slice(0, 80),
            description: [
                (rule as any).rule ? `Reviewer rule: ${(rule as any).rule}` : 'Accepted reviewer replacement.',
...
const LAYER_ORDER: ManifestLayer[] = [
    'pre_ai_deterministic',
    'prompt_section',
    'parse_normalization',
    'post_ai_guard',
...
const LAYER_TITLES: Record<ManifestLayer, string> = {
...
export function renderRulesManifestMarkdown(manifest: ReviewRulesManifest, opts: { includeDynamic?: boolean } = {}): string {
    const lines: string[] = [
        '# Code Rules Manifest',
        '',
    ...
    for (const layer of LAYER_ORDER) {
        ...
        if (replacementEntries.length) {
            ...
            for (const entry of replacementEntries) {
                const data = entry.data as { from: string; to: string; type: string };
