...
export type EmbeddedSetMenuSection = {
    title: string;
...
export type SetMenuItemPriceIssue = {
    sectionTitle: string;
    lineIndex: number;
    line: string;
    menuItem: string;
...
export type EmbeddedSetMenuAnalysis = {
    sections: EmbeddedSetMenuSection[];
    issues: SetMenuItemPriceIssue[];
};

export type EmbeddedSetMenuGuardResult = {
    correctedMenu: string;
    suggestions: BasicSuggestion[];
    restoredPrices: SetMenuItemPriceIssue[];
    synthesizedSuggestions: BasicSuggestion[];
...
function isPackageTitleLine(line: string): boolean {
...
function isChoiceInstructionLine(line: string): boolean {
    const normalized = normalizeForMatch(line);
...
function isSetOptionHeading(line: string): boolean {
    const trimmed = `${line || ''}`.replace(/\s+/g, ' ').trim();
...
function hasNearbyChoiceInstruction(lines: string[], titleIndex: number): number {
...
function hasSetHeadingAfterChoice(lines: string[], choiceIndex: number): boolean {
...
function findSectionEnd(lines: string[], titleIndex: number): number {
...
function lineHasPremiumPrice(line: string): boolean {
    return PREMIUM_PRICE_TAIL_REGEX.test(`${line || ''}`.trim());
}

function extractBarePrice(line: string): string | null {
    const trimmed = `${line || ''}`.replace(/\s+/g, ' ').trim();
...
function stripTrailingDietarySuffix(line: string): string {
    return `${line || ''}`
...
function extractMenuItem(line: string, price: string): string {
...
function isSkippableSectionLine(line: string): boolean {
    return !`${line || ''}`.trim()
...
function collectIssues(lines: string[], section: EmbeddedSetMenuSection): SetMenuItemPriceIssue[] {
    const issues: SetMenuItemPriceIssue[] = [];
...
export function analyzeEmbeddedSetMenus(menuText: string): EmbeddedSetMenuAnalysis {
    const lines = splitLines(menuText);
    const sections: EmbeddedSetMenuSection[] = [];
    const issues: SetMenuItemPriceIssue[] = [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const section: EmbeddedSetMenuSection = {
...
export function buildEmbeddedSetMenuPromptSection(analysis: EmbeddedSetMenuAnalysis): string {
...
function issueKey(issue: SetMenuItemPriceIssue): string {
    return `${normalizeForMatch(issue.sectionTitle)}::${normalizeForMatch(issue.menuItem)}::${normalizePrice(issue.price)}`;
}

function suggestionMatchesIssue(suggestion: BasicSuggestion, issue: SetMenuItemPriceIssue): boolean {
...
function isSetPriceLikeSuggestion(suggestion: BasicSuggestion): boolean {
...
function isMissingPriceSuggestion(suggestion: BasicSuggestion): boolean {
    const type = normalizeForMatch(suggestion.type || '');
    const combined = normalizeForMatch(`${suggestion.description || ''} ${suggestion.recommendation || ''}`);
    return type.includes('missing price') || /(?:missing|no).*price/.test(combined);
}

function findLineForSuggestionInOriginal(suggestion: BasicSuggestion, originalLines: string[], analysis: EmbeddedSetMenuAnalysis): string | null {
...
function findCorrectedLineIndex(correctedLines: string[], issue: SetMenuItemPriceIssue, usedIndexes: Set<number>): number {
    const itemNorm = normalizeForMatch(issue.menuItem);
    if (!itemNorm) {
        return -1;
    }
...
function removeTrailingPremiumPrice(line: string, issue: SetMenuItemPriceIssue): string {
...
function restoreBarePrice(line: string, issue: SetMenuItemPriceIssue): string {
...
function synthesizeSetMenuItemPriceSuggestion(issue: SetMenuItemPriceIssue): BasicSuggestion {
...
function normalizeExistingSetMenuSuggestion(suggestion: BasicSuggestion): BasicSuggestion {
    return {
        ...suggestion,
        type: 'Set Menu Item Price',
...
export function guardEmbeddedSetMenuPrices(
    originalMenu: string,
    correctedMenu: string,
    suggestions: BasicSuggestion[],
    existingAnalysis?: EmbeddedSetMenuAnalysis
): EmbeddedSetMenuGuardResult {
    ...
    const keptSuggestions: BasicSuggestion[] = [];
    const synthesizedSuggestions: BasicSuggestion[] = [];
    const droppedSuggestions: EmbeddedSetMenuGuardResult['droppedSuggestions'] = [];
    ...
    const usedCorrectedIndexes = new Set<number>();
    const restoredPrices: SetMenuItemPriceIssue[] = [];
