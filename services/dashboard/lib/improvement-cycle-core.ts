...
} | null | undefined): PromptProposalApprovalBlock | null {
...
export type ImprovementLlmOutput = {
    ...
    warnings: string[];
    // Fix 2: set when still_missed corrections have no covering replacement rule,
    // code recommendation, or prompt change near the correction.
...
export function isReasoningModel(model: string): boolean {
    return adapterIsReasoningModel(model);
}
 ...
 * sampling controls.
 */
export function buildReplayRequestBody(opts: {
    model: string;
    temperature: number;
    seed: number;
    prompt: string;
    ...
    if (!isReasoningModel(opts.model)) body.temperature = opts.temperature;
    return body;
}
...
export function resolveEvalBaselineModel(candidateModel: string, baselineModel?: string): string {
...
export type AtomicExtractionCandidate = {
    original_text: string;
    corrected_text: string;
    change_type: string;
    /** Rows this pair was recovered from, for provenance on the review page. */
    source_rule_ids: string[];
    occurrences: number;
};

export type AtomicExtractionReport = {
    candidates: AtomicExtractionCandidate[];
    /** Pairs seen in BOTH directions — self-evidence that the term is context-dependent. */
    contextDependent: Array<{ a: string; b: string; reason: string }>;
    /** Recovered pairs an atomic rule already covers. */
    alreadyCovered: Array<{ original_text: string; corrected_text: string }>;
    /** Full-line rows nothing could be recovered from, with why. */
...
export function isAtomicRuleText(text: string): boolean {
 ...
 *   are filtered out
 */
export function extractAtomicRulesFromCorpus(rules: ExistingCorrectionRule[]): AtomicExtractionReport {
    const report: AtomicExtractionReport = { candidates: [], contextDependent: [], alreadyCovered: [], skipped: [] };
    ...
    const recovered = new Map<string, AtomicExtractionCandidate>();
    for (const row of rows) {
        const id = row.id ? String(row.id) : null;
        if (isAtomicRuleText(row.original_text || '')) continue; // already usable as-is
        ...
        if (existing) {
            ...
            if (id) existing.source_rule_ids.push(id);
            continue;
        }
        recovered.set(key, {
...
export function validateImprovementLlmOutput(
    ...
    if (!promptUnchanged && opts.currentPrompt) {
        ...
        const hasResponseFenceContract = (text: string) => Object.values(AI_REVIEW_FENCES).every((marker) => text.includes(marker));
    ...
    const meaningfulCorrectionTokens = (value: unknown): string[] => {
        const tokens = `${value ?? ''}`.toLowerCase().match(/[\p{L}\p{N}]{4,}/gu) || [];
        return [...new Set(tokens.filter((token) => !promptCoverageStopwords.has(token)))];
    };
    const promptChangedNearCorrection = (originalText: string, correctedText: string): boolean => {
        if (!changedPromptHay) return false;
    ...
    const normalizeCoverageText = (value: string): string => value.toLowerCase().replace(/\s+/g, ' ').trim();
    const escapeCoverageRegExp = (value: string): string => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const rulesFullyTransformCorrection = (originalText: string, correctedText: string): boolean => {
    ...
    const sourceByCorrectionId = new Map(
        (opts.sourceCorrections || [])
    ...
    for (const e of stillMissed) {
        ...
        const recommendationHits = !!recommendationHay && (
            ((o || c) && ((o && recommendationHay.includes(o)) || (c && recommendationHay.includes(c))))
...
export function resolveTriggerEvalCaseId(
    submissionId: string,
    caseIdsBySubmission: Record<string, string> | null | undefined
): string {
...
export function evalStatusFromSummary(
    ...
    if (opts.rulesOnly) {
        const activations = Array.isArray(summary.candidate_rule_activations)
            ...
            : [];
        if (
            Number(summary.comparedCases || 0) > 0
            && activations.length > 0
            && activations.every((entry) => Number(entry.total_activations || 0) > 0)
        ) {
            return 'passed';
...
export function buildImprovementSystemPrompt(opts: { executorModel: string }): string {
    return `You are the review-process engineer for an AI menu editor at ${getTenantConfig().name} (${getTenantConfig().shortName}).
 ...
 */
export function buildConsolidationSystemPrompt(opts: { executorModel: string }): string {
    return `You are the review-process engineer for an AI menu editor at ${getTenantConfig().name} (${getTenantConfig().shortName}).
