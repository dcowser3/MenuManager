...
function assessCodeProposalVerificationInternal(proposal, allowTestOnly) {
    ...
    if (deliveryIds.size > 0 && proof.schema_version >= 2 && corrections.some((correction) => deliveryIds.has(correction.correction_id) && correction.delivery_evidence_scope !== 'serializer_request_boundary_v1'))
        return fail('delivery evidence is limited to the frozen serializer request-boundary scope.');
