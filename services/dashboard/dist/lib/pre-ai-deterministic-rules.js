 ...
 */
exports.CURATED_CANONICAL_FOOD_WORDS = [
...
function boundedDamerauDistance(left, right, maxDistance) {
...
function normalizeCuratedFoodSpellingsOnLine(line, lineIndex) {
    const corrections = [];
    ...
    const nextLine = line.replace(tokenPattern, (found) => {
        ...
        const candidates = exports.CURATED_CANONICAL_FOOD_WORDS
            .map((entry) => ({
            entry,
            ...
            .filter(({ entry, canonicalFolded, distance }) => (distance <= entry.maxDistance
...
function isGlobalRuleLocation(value) {
    const normalized = normalizeScope(value);
    return !normalized || normalized === 'all properties global rule';
}
function ruleAppliesToProperty(rule, property) {
...
function titleCaseLike(value) {
    return value.replace(/[A-Za-zÀ-ÖØ-öø-ÿ]+/g, (word) => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase());
}
...
function replacementRegExp(from, separatorVariants = false, acceptedTarget = '') {
    ...
    if (separatorVariants) {
        ...
        if (sourceChars.length > 0 && sourceCore === targetCore) {
            ...
            escaped = sourceChars.map((char, index) => `${escapeRegExp(char)}${index < sourceChars.length - 1 && boundaries.has(index + 1)
                ? '[ \\u00a0\\u2010\\u2011-]*'
                : ''}`).join('');
        }
    }
...
function applyReplacementRule(line, lineIndex, rule, source, metadata = {}, settings = {}) {
    ...
    const nextLine = line.replace(re, (match, offset) => {
        ...
        if (settings.skipIfAlreadyCorrected
...
function normalizeContextualCompoundDescriptorsOnLine(line, lineIndex) {
    const corrections = [];
    ...
    nextLine = nextLine.replace(/\b([A-Za-zÀ-ÖØ-öø-ÿ]+(?:[- ][A-Za-zÀ-ÖØ-öø-ÿ]+)?)\s+(grilled)\s+([A-Za-zÀ-ÖØ-öø-ÿ]+)/giu, (match, modifier, participle, noun, offset) => {
    ...
    nextLine = nextLine.replace(/\b(brûlée)\s+([A-Za-zÀ-ÖØ-öø-ÿ]+)/giu, (match, source, ingredient, offset) => {
        const preceding = nextLine.slice(0, offset).trimEnd().toLowerCase();
        const ingredientKey = stripDiacritics(ingredient).toLowerCase();
        if (!BRULEE_INGREDIENTS.has(ingredientKey) || /(?:^|\s)crème$/.test(preceding) || BRULEE_LEXICAL_DESSERTS.has(ingredientKey))
            return match;
...
function normalizeAllergenClusterOnLine(line, lineIndex, validCodes) {
    const original = line || '';
    if (!original.trim()) {
...
function ensureTresLechesVegetarianCodeOnLine(line, lineIndex, validCodes) {
    const original = line || '';
...
function ensureCotijaCheeseModifierOnLine(line, lineIndex) {
    ...
    const corrected = original.replace(pattern, (match, offset) => {
        if (firstDescriptionComma < 0 || offset <= firstDescriptionComma)
            return match;
        if (!/^cotija$/i.test(match) && !frozenNamedCheeseContext)
            return match;
 ...
 */
function isVerifiedMixedSalmonOptionLine(line) {
    if (!line || !/,/.test(line))
        return false;
...
function addInteriorSalmonOptionMarker(line) {
    if (!isVerifiedMixedSalmonOptionLine(line) || line.includes('*'))
        return line;
    const parts = line.split(',');
    let changed = false;
...
function getAcceptedCorrectionRulePreAiEligibility(rule) {
    ...
    const contextTerm = (0, improvement_cycle_core_1.involvesContextDependentTerm)(original, corrected);
    if (contextTerm) {
