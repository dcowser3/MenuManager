...
exports.describeServiceError = describeServiceError;
exports.withSubmissionReference = withSubmissionReference;
exports.parseJsonLike = parseJsonLike;
exports.normalizeRawPayload = normalizeRawPayload;
exports.buildClickUpTaskPayloadFromStoredSubmission = buildClickUpTaskPayloadFromStoredSubmission;
exports.mergeClickUpHandoffMetadata = mergeClickUpHandoffMetadata;
...
function withSubmissionReference(message, submissionId) {
    return submissionId ? `${message} Reference: ${submissionId}.` : message;
}
function parseJsonLike(value, fallback = undefined) {
...
function buildClickUpTaskPayloadFromStoredSubmission(submission, assets = []) {
    const raw = normalizeRawPayload(submission.raw_payload);
    const source = { ...raw, ...submission };
    const findAsset = (assetType) => assets.find((asset) => asset.asset_type === assetType);
...
function mergeClickUpHandoffMetadata(rawPayload, metadata) {
