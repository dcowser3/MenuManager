...
exports.analyzeFullLineCorrectionProgress = analyzeFullLineCorrectionProgress;
exports.analyzeReplayCorrection = analyzeReplayCorrection;
...
exports.shouldSkipCandidateEval = shouldSkipCandidateEval;
exports.resolveDashboardPublicUrl = resolveDashboardPublicUrl;
exports.isLoopbackBaseUrl = isLoopbackBaseUrl;
exports.describePublicUrlMisconfiguration = describePublicUrlMisconfiguration;
exports.isBareIpBaseUrl = isBareIpBaseUrl;
...
function isGuardDiscardReason(reason) {
 ...
 */
function isRequestTooLarge429(bodyText) {
...
function validateCorrectionRouting(rawRouting, opts = {}) {
    ...
    for (const value of Array.isArray(rawRouting) ? rawRouting : []) {
        ...
        let lane = asText(entry.lane, 40).toLowerCase();
        if (!exports.CORRECTION_ROUTING_LANES.has(lane)) {
    ...
    for (const c of sources) {
        ...
        if (e) {
            ordered.push(e);
            seen.add(e.correction_id);
        }
    }
    for (const e of byId.values()) {
        if (!seen.has(e.correction_id))
            ordered.push(e);
    }
 ...
 */
function minimalChangedSpan(originalText, correctedText) {
    const original = `${originalText || ''}`;
    const corrected = `${correctedText || ''}`;
    if (!original || !corrected || original === corrected)
        return null;
    let prefix = 0;
    while (prefix < original.length && prefix < corrected.length && original[prefix] === corrected[prefix])
        prefix++;
    ...
    while (prefix > 0 && isWordCharacter(original[prefix - 1])
        && (isWordCharacter(original[prefix]) || isWordCharacter(corrected[prefix])))
        prefix--;
    let suffix = 0;
    ...
    while (suffix > 0) {
        ...
        if (isWordCharacter(original[oi]) && (isWordCharacter(original[oi - 1]) || isWordCharacter(corrected[ci - 1])))
            suffix--;
        else
            break;
    }
...
function validateImprovementLlmOutput(raw, opts = {}) {
    ...
    if (!isConsolidation && (opts.sourceCorrections || []).some((c) => c && c.id)) {
        ...
        for (const w of routed.warnings)
            warnings.push(w);
        if (routed.unresolvedFromRouting)
            unresolvedStillMissed = true;
...
function buildGuardRetryMessage(params) {
    return [
...
async function runImprovementProposalWithRetry(params) {
    const maxRetries = Number.isFinite(Number(params.maxRetries)) ? Math.max(0, Number(params.maxRetries)) : 2;
    const totalAttempts = maxRetries + 1;
    const parseJson = params.parseJson || ((raw) => JSON.parse(raw));
    const messages = [
        { role: 'system', content: params.systemPrompt },
        { role: 'user', content: params.userPrompt },
    ];
    const attempts = [];
    const discardedPrompts = [];
    ...
    for (let attempt = 1; attempt <= totalAttempts; attempt++) {
        ...
        const rawProposed = asText(parsed?.proposed_prompt);
        ...
        if (guardDiscarded)
            discardedPrompts.push(rawProposed);
        ...
        if (attemptWarnings.length > 1) {
            ...
            for (let i = 0; i < attemptWarnings.length; i++) {
                for (const w of attemptWarnings[i])
                    merged.push(`attempt ${i + 1}/${totalAttempts}: ${w}`);
            }
...
function multisetIntersection(expected, actual) {
    const counts = new Map();
    for (const token of actual)
        counts.set(token, (counts.get(token) || 0) + 1);
    const matched = [];
...
function multisetDifference(expected, matched) {
    const counts = new Map();
...
function describeTokenChanges(deleted, inserted) {
    return [
 ...
 */
function analyzeFullLineCorrectionProgress(originalText, correctedText, replayOutput) {
    ...
    for (const line of replayLines) {
        ...
        best = {
            ...
            remaining_changes: describeTokenChanges(multisetDifference(expectedDeleted, matchedDeleted), multisetDifference(expectedInserted, matchedInserted)),
        };
    }
    return best;
}
/** Return the replay verdict plus partial-progress detail for proposal context/UI. */
function analyzeReplayCorrection(originalText, correctedText, replayOutput, signals) {
    ...
    if (lineLevel) {
        ...
        if (progress.complete)
            return { status: 'now_correct', applied_changes: progress.applied_changes };
        if (progress.partial) {
            return {
...
function shouldAttemptRulesOnlyFallback(input) {
    ...
    if (!attribution || attribution.status !== 'completed' || !attribution.cases.length)
        return false;
    if (input.unresolvedStillMissed)
        return false;
    if (attribution.cases.some((entry) => entry.cause !== 'prompt'))
        return false;
    ...
    if (!routing.length || routing.some((entry) => entry.lane === 'prompt' || entry.lane === 'unrouted'))
        return false;
    return true;
}
 ...
 */
function buildTriggerProgressionFromReports(input) {
    const baselineByCase = new Map((input.baselineReport?.cases || []).map((entry) => [`${entry.case_id}`, entry]));
    const candidateByCase = new Map((input.candidateReport?.cases || []).map((entry) => [`${entry.case_id}`, entry]));
    ...
    for (const list of [comparison?.improvements || [], comparison?.regressions || [], comparison?.noiseRegressions || []]) {
        for (const entry of list)
            if (entry?.case_id)
                comparisonByCase.set(`${entry.case_id}`, entry);
    }
    const findCase = (map, submissionId) => {
        ...
        for (const [caseId, entry] of map.entries()) {
            if (caseId === `production:${submissionId}` || caseId.endsWith(`:${submissionId}`))
                return entry;
        }
        return null;
    };
...
function computeDisposition(input) {
...
function shouldSkipCandidateEval(input) {
    return !!input.promptUnchanged && (input.proposedRuleCount || 0) === 0;
}
exports.IDENTICAL_CANDIDATE_EVAL_NOTE = 'eval skipped: candidate identical to baseline';
 ...
 */
function describePublicUrlMisconfiguration(env) {
