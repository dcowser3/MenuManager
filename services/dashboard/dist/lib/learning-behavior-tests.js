...
exports.validateBehaviorArtifact = validateBehaviorArtifact;
exports.executeBehaviorTests = executeBehaviorTests;
...
function buildBehaviorTestRecord(correction, evidence = {}) {
    ...
    const classification = menuUpdate ? 'menu_content_update'
...
function approvedCase(source, target, force) {
    if (force)
        return target;
    ...
    if (words.length && words.every(word => word[0] === word[0].toUpperCase()))
        return target.replace(/\p{L}+/gu, word => word[0].toUpperCase() + word.slice(1).toLowerCase());
    return target;
}
/** Only labels mechanically justified by an accepted policy; semantic neighbors are abstention controls. */
function buildAcceptedPolicyTestFamily(rule) {
    if (rule.status !== 'accepted' || !rule.original_text || !rule.corrected_text
        || !(0, pre_ai_deterministic_rules_1.getAcceptedCorrectionRulePreAiEligibility)(rule).eligible
        || ['menu_content_update', 'menu_update_only'].includes(rule.learning_intent || rule.change_type || ''))
        return [];
    const original = rule.original_text, target = rule.corrected_text;
    const cases = [];
    if ((0, canonical_policy_1.permitsSeparatorVariants)(original, target)) {
    ...
    for (const quantity of ['12', '120', '1/2'])
        cases.push({ input: `${original} ${quantity}`, expected: `${target} ${quantity}`, kind: 'quantity_preservation' });
    for (const example of rule.reviewed_negative_examples || []) {
    ...
    if (rule.is_location_specific)
        cases.push({ input: original, expected: original, kind: 'wrong_scope_abstention', scope: 'outside_policy' });
...
function freezeBehaviorTests(records, policies, allPolicies = policies) {
    ...
    const eligiblePolicies = policies.filter(rule => !excluded.has(rule.id) && !['menu_content_update', 'menu_update_only'].includes(rule.learning_intent || rule.change_type || ''));
    const tests = eligiblePolicies.flatMap(rule => {
        ...
        if (!(0, canonical_policy_1.resolveCanonicalPolicies)(allPolicies.filter(row => !excluded.has(row.id)), context).rules.some(row => row.id === rule.id))
            return [];
        return buildAcceptedPolicyTestFamily(rule).map((test, index) => {
            ...
            return { ...test, expected: test.scope === 'outside_policy'
                    ? (0, pre_ai_deterministic_rules_1.canonicalizeFinalTerms)(test.input, { ...testContext, acceptedCorrectionRules: allPolicies.filter(row => !excluded.has(row.id)) }).menuText : test.expected,
...
function validateBehaviorArtifact(artifact) {
    if (!artifact || artifact.schemaVersion !== 1 || !Array.isArray(artifact.records) || !Array.isArray(artifact.tests)
        || artifact.tests.length > 5000 || !Number.isFinite(Date.parse(artifact.frozenAt)))
        throw new Error('Missing trusted pre-draft behavior tests.');
    const { sha256, ...body } = artifact;
    if (hash(body) !== sha256)
        throw new Error('Trusted behavior expectations changed after freezing.');
    return artifact;
}
async function executeBehaviorTests(artifact, evaluate) {
    ...
    for (const test of artifact.tests) {
        try {
            ...
            outcomes.push({ id: test.id, correctionId: test.correctionId, kind: test.kind, passed: false, error: error.message });
        }
    }
    return { artifactHash: artifact.sha256, passed: outcomes.every(test => test.passed), outcomes,
        explanations: artifact.records.map((record) => ({ correctionId: record.correctionId,
