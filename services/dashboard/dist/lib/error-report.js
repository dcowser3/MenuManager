...
exports.shouldRunErrorReportAiTriage = shouldRunErrorReportAiTriage;
exports.buildErrorReportEmail = buildErrorReportEmail;
exports.buildErrorReportTriagePrompt = buildErrorReportTriagePrompt;
exports.buildErrorReportTriageEmail = buildErrorReportTriageEmail;
...
function shouldRunErrorReportAiTriage(env = process.env) {
...
function compactForPrompt(value, depth = 0) {
    ...
    if (typeof value === 'number' || typeof value === 'boolean')
        return value;
    if (Array.isArray(value))
        return value.slice(0, 40).map((item) => compactForPrompt(item, depth + 1));
    if (typeof value === 'object') {
        return Object.fromEntries(Object.entries(value)
            .slice(0, 80)
...
function buildErrorReportTriagePrompt(report, incident) {
...
function buildErrorReportTriageEmail(report, incident, proposal, options) {
    const subjectTarget = report.projectName || report.property || report.submitterEmail || report.attemptId;
