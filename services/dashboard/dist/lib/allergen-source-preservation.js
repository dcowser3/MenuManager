...
function rowEvidence(source, candidate) {
    ...
    const available = [...candidateTokens];
    let matched = 0;
...
function preserveSubmittedAllergenCodes(submittedMenu, candidateMenu, allergenLegend) {
    const validCodes = configuredCodes(allergenLegend);
    if (!validCodes.size || !submittedMenu || !candidateMenu)
        return { menuText: candidateMenu, diagnostics: [], restoredRows: 0 };
    ...
    for (const source of sourceRows) {
        ...
        if (matches.length !== 1) {
            ...
            const best = fuzzy[0];
            const tied = best && fuzzy.filter(item => best.score - item.score < 0.05);
    ...
    candidateLines.forEach((line, index) => {
        ...
        restoredRows += 1;
        diagnostics.push(`model_allergen_codes_removed:unmatched:${index}`);
    });
