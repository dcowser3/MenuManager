...
const ERROR_REPORT_CLIENT_MAX_BODY_BYTES = Math.max(1024 * 1024, parseBodyLimitBytes(ERROR_REPORT_JSON_BODY_LIMIT, 15 * 1024 * 1024) - ERROR_REPORT_BODY_SAFETY_BYTES);
...
function parseBodyLimitBytes(value, fallback) {
...
function truncateDiagnosticText(value) {
    ...
    if (text.length <= BASIC_AI_CHECK_DEBUG_MAX_CHARS)
        return text;
    return `${text.slice(0, BASIC_AI_CHECK_DEBUG_MAX_CHARS)}\n...[truncated ${text.length - BASIC_AI_CHECK_DEBUG_MAX_CHARS} chars]`;
}
function wantsBasicCheckDiagnostics(req) {
    if (!BASIC_AI_CHECK_DEBUG_ENABLED)
        return false;
    ...
    void (async () => {
        ...
        if (submitterRecipients.length) {
            sends.push({
                ...
                cc: (0, submission_confirmation_mail_1.buildSubmissionConfirmationCc)(submitterRecipients[0], [], tenantConfig.emails.submissionConfirmationCc),
        ...
        if (approverRecipients.length) {
            ...
            sends.push({
                ...
                cc: (0, submission_confirmation_mail_1.buildSubmissionConfirmationCc)(primaryApprover, ccApprovers, submitterRecipients.length ? [] : tenantConfig.emails.submissionConfirmationCc),
        ...
        for (const send of sends) {
            try {
                ...
                const result = await (0, alert_mail_1.sendAlertMail)({
                    ...
                    html: (0, submission_confirmation_mail_1.buildSubmissionReceiptHtml)(input, attachments.length === 0, DASHBOARD_URL, { includeDisputeLink: send.includeDisputeLink }),
                    attachments,
                }, alertMailDeps);
...
async function findLocalApprovedDocx(approvedMenu) {
...
async function fetchSharePointApprovedDocx(submissionId) {
    try {
        const response = await internalApi.get(`${CLICKUP_SERVICE_URL}/sharepoint/file?submissionId=${encodeURIComponent(submissionId)}`, { responseType: 'arraybuffer', timeout: 30000 });
...
async function materializeSharePointApprovedDocx(approvedMenu, contents) {
    const approvedDir = path.join(getSubmissionDocumentDir(approvedMenu.projectName, approvedMenu.property, approvedMenu.id), 'approved');
    const approvedPath = path.join(approvedDir, `${approvedMenu.id}-approved.docx`);
    await fs_1.promises.mkdir(approvedDir, { recursive: true });
    await fs_1.promises.writeFile(approvedPath, contents);
    await internalApi.put(`${DB_SERVICE_URL}/submissions/${encodeURIComponent(approvedMenu.id)}`, {
        final_path: approvedPath,
    });
    return approvedPath;
}
async function regenerateCleanApprovedDocx(approvedMenu) {
    ...
    await generateDocxFromForm(approvedMenu.id, {
        ...
        menuContentHtml: menuContentHtml || (0, approval_baseline_1.textToParagraphHtml)(menuContent),
        allergens: approvedMenu.allergens || rawPayload.allergens || DEFAULT_ALLERGEN_KEY,
    }, { outputPath });
    return outputPath;
}
async function resolveApprovedDocxDownload(approvedMenu, allowRegeneration) {
    return (0, approved_download_1.resolveApprovedDownload)({
...
async function getPropertyCatalogFromDb() {
    const fallbackCatalog = (0, property_catalog_1.buildFallbackPropertyCatalog)();
    ...
    try {
        ...
        const catalog = raw
            .map((item) => {
            const normalized = (0, property_catalog_1.normalizePropertyCatalogRecord)(item);
...
app.use('/api/form/error-report', express_1.default.json({ limit: ERROR_REPORT_JSON_BODY_LIMIT }));
...
async function createCleanApprovedDocx(sourcePath, submissionId) {
    const docxRedlinerDir = getDocxRedlinerDir();
    const venvPython = path.join(docxRedlinerDir, 'venv', 'bin', 'python');
    ...
    const tempDir = await fs_1.promises.mkdtemp(path.join(getRepoRoot(), 'tmp', 'approved-clean-'));
...
function buildCleanApprovedDownloadName(fileName) {
    const sanitized = (0, upload_security_1.sanitizeStoredFileName)(fileName || 'approved-menu.docx', 'approved-menu.docx');
...
const draftTokenLookupAttempts = new Map();
function isDraftTokenLookupRateLimited(req) {
...
async function loadDraftSessionForForm(req) {
    const token = (0, upload_security_1.sanitizePlainTextInput)(req.query?.draft, { maxLength: 160 }).trim();
    if (!token) {
...
async function renderSubmissionForm(req, res, view, title) {
    ...
    res.render(view, {
        ...
        maxUploadBytes: upload_security_1.MAX_UPLOAD_BYTES,
...
app.get('/approved-menus', async (req, res) => {
    try {
        ...
        const restaurant = (0, upload_security_1.sanitizePlainTextInput)(req.query.restaurant, { maxLength: 120 }).trim();
...
app.get('/approval-dispute/:token', async (req, res) => {
    ...
    const token = (0, upload_security_1.sanitizePlainTextInput)(req.params.token, { maxLength: 120 }).trim();
    try {
...
app.post('/approval-dispute/:token', async (req, res) => {
    ...
    const token = (0, upload_security_1.sanitizePlainTextInput)(req.params.token, { maxLength: 120 }).trim();
    const note = (0, upload_security_1.sanitizePlainTextInput)(req.body?.note, { multiline: true, maxLength: 500 }).trim();
    try {
        ...
        if (tenantConfig.emails.internalReviewer) {
            ...
            Promise.resolve((0, alert_mail_1.sendAlertMail)({
                fromName: 'Menu Manager',
            ...
            }, alertMailDeps)).catch((mailError) => console.error('Dispute reviewer notification failed:', mailError?.message || mailError));
        }
        return res.render('approval-dispute', {
            token,
            state: 'recorded',
...
app.get('/download/approved-clean/:submissionId', async (req, res) => {
    ...
    try {
        ...
        const downloadName = buildCleanApprovedDownloadName(approvedMenu.approvedFileName || approvedMenu.filename || path.basename(absolutePath));
        ...
        return res.download(cleaned.filePath, downloadName, (downloadError) => {
            fs_1.promises.rm(cleanupDir, { recursive: true, force: true }).catch((cleanupError) => {
        ...
        if (cleanupDir) {
            fs_1.promises.rm(cleanupDir, { recursive: true, force: true }).catch(() => undefined);
        }
...
async function runSubmissionReviewThroughCoordinator(input, overrides = {}) {
    ...
    const prepared = await (0, review_pipeline_1.prepareReview)(input.text, {
        basePrompt: qaPrompt,
    ...
    const responseValidation = (0, review_contract_1.validateCoordinatorResponse)(data);
    ...
    if (responseIdentity.requestDigest !== coordinatorRequest.requestDigest
        ...
        || !(0, review_contract_1.sameExecutionIdentity)(responseIdentity.effectiveExecutionIdentity, effectiveExecutionIdentity)
...
app.post('/api/learning/correction-rules', async (req, res) => {
    try {
        ...
        const trustedPayload = (0, learning_correction_rules_1.requiresHumanExplanationSourceBinding)(payload)
        ...
        const record = (0, learning_correction_rules_1.buildCorrectionRuleRecord)(trustedPayload, catalog);
...
app.post('/api/learning/prompt-proposal/:id/review', async (req, res) => {
    try {
        ...
        const supersededBlock = (0, improvement_cycle_core_1.supersededProposalReviewBlock)(proposalRecord);
        if (supersededBlock) {
        ...
        for (const [position, rule] of acceptedRules.entries()) {
            ...
            try {
                const payload = (0, improvement_cycle_core_1.mapProposedRuleToCorrectionRulePayload)(rule, proposalRecord?.cycle_id || id, index, reviewer_name || null, {
...
app.post('/api/drafts', async (req, res) => {
    try {
        const baseSubmissionId = (0, upload_security_1.sanitizePlainTextInput)(req.body?.baseSubmissionId || req.body?.base_submission_id, { maxLength: 120 }).trim();
        if (!baseSubmissionId) {
            return res.status(400).json({ error: 'baseSubmissionId is required' });
        }
        const replaceToken = (0, upload_security_1.sanitizePlainTextInput)(req.body?.replaceToken || req.body?.replace_token, { maxLength: 160 }).trim();
        const draftRequest = { baseSubmissionId };
        if (replaceToken)
            draftRequest.replaceToken = replaceToken;
...
app.post('/api/drafts/:token/discard', async (req, res) => {
    try {
        const token = (0, upload_security_1.sanitizePlainTextInput)(req.params.token, { maxLength: 160 }).trim();
        if (!token)
 ...
 */
const errorReportCooldowns = new Map();
const ERROR_REPORT_COOLDOWN_MS = 15 * 1000;
function getErrorReportsDir() {
    return path.join(getRepoRoot(), 'tmp', 'error-reports');
}
function generateErrorReportIncidentId() {
    const timestamp = new Date().toISOString()
        .replace(/[-:]/g, '')
        .replace(/\.\d{3}Z$/, 'Z');
    return `err-${timestamp}-${crypto_1.default.randomUUID().slice(0, 8)}`;
}
async function requestErrorReportAiTriage(report, incident) {
    if (!(0, error_report_1.shouldRunErrorReportAiTriage)()) {
        return '';
    }
...
async function sendErrorReportAiTriageEmail(report, incident) {
    if (!(0, alert_mail_1.canSendAlertMail)(alertMailDeps) || !FORM_ATTEMPT_ALERT_EMAIL || !(0, error_report_1.shouldRunErrorReportAiTriage)()) {
        return;
    }
    ...
    const { subject, html } = (0, error_report_1.buildErrorReportTriageEmail)(report, incident, proposal, {
        model: ERROR_REPORT_TRIAGE_MODEL,
...
app.post('/api/form/error-report', async (req, res) => {
    try {
        const report = (0, error_report_1.normalizeErrorReport)(req.body);
        ...
        const screenshot = (0, error_report_1.decodeScreenshotDataUrl)(req.body?.screenshotDataUrl);
        ...
        try {
            ...
            await fs_1.promises.writeFile(path.join(reportDir, 'report.json'), JSON.stringify({
                incidentId,
                savedAt: new Date().toISOString(),
                ...summary,
            }, null, 2));
            await fs_1.promises.writeFile(path.join(reportDir, 'client-state.json'), stateJson);
            if (screenshot) {
                await fs_1.promises.writeFile(path.join(reportDir, `screenshot.${screenshot.extension}`), screenshot.buffer);
            }
            savedTo = reportDir;
        }
        catch (saveError) {
...
async function handleBasicCheck(req, res) {
    ...
    const basicCheckId = (0, upload_security_1.sanitizePlainTextInput)(req.get?.('x-menumanager-basic-check-id'), { maxLength: 100 }) || crypto_1.default.randomUUID();
    ...
    try {
        ...
        if (!changedOnlyMode) {
            coordinatorPreparation = await (0, review_pipeline_1.prepareReview)(menuContent, {
                basePrompt: qaPrompt,
                property,
                templateType,
                menuType,
        ...
        const nearMissAnalysis = coordinatorPreparation?.nearMissAnalysis || await (0, canonical_vocabulary_provider_1.buildNearMissAnalysis)(preCheckedReviewBody, {
        ...
        const promptInfo = coordinatorPreparation?.promptInfo || (0, qa_prompt_builder_1.buildFinalPrompt)(qaPrompt, {
            property,
            templateType,
            menuType,
            acceptedCorrectionRules,
        ...
        const buildBasicCheckAuditEvent = (eventType, statusCode, extra = {}) => ({
            attemptId,
            checkId: basicCheckId,
            eventType,
            route: '/api/form/basic-check',
        ...
        try {
            ...
            await (0, basic_ai_check_audit_1.logBasicAiCheckAudit)(buildBasicCheckAuditEvent('ai_unavailable', 200, {
                correctedMenuTextLength: deterministicFallbackMenu.length,
                suggestionsCount: 0,
                criticalSuggestionsCount: 0,
        ...
        if (typeof feedback !== 'string' || !feedback.trim()) {
            ...
            await (0, basic_ai_check_audit_1.logBasicAiCheckAudit)(buildBasicCheckAuditEvent('malformed_response', 200, {
                correctedMenuTextLength: deterministicFallbackMenu.length,
                suggestionsCount: 0,
                criticalSuggestionsCount: 0,
                aiRequest: buildAiRequestAudit(),
        ...
        const coordinatedResult = coordinatorPreparation
            ? (0, review_pipeline_1.completePreparedReview)(coordinatorPreparation, feedback, { finishReason: qaResponse?.data?.finish_reason })
            : null;
...
async function runBasicCheckJob(checkId, body, headers) {
...
app.get('/api/form/basic-check/status/:checkId', (req, res) => {
    cleanupBasicCheckJobs();
    const checkId = (0, upload_security_1.sanitizePlainTextInput)(req.params.checkId, { maxLength: 100 });
    const job = basicCheckJobs.get(checkId);
    if (!job) {
...
async function syncEffectivePromptFromDb() {
    try {
        if (!(0, supabase_client_1.isSupabaseConfigured)())
            return;
        const supabase = (await Promise.resolve().then(() => __importStar(require('@menumanager/supabase-client')))).getSupabaseClient();
        const { data: approvedProposals, error } = await supabase
            ...
            .limit(5);
        if (error || !approvedProposals?.length)
            return;
        const qaPromptPath = path.join(getRepoRoot(), 'sop-processor', 'qa_prompt.txt');
...
app.use((error, req, res, next) => {
    ...
    if (error instanceof multer_1.default.MulterError) {
        ...
        const clientMessage = isFileSize
            ? (0, upload_security_1.buildUploadTooLargeMessage)(upload_security_1.MAX_UPLOAD_BYTES, PUBLIC_FORM_SUPPORT_EMAIL)
            : `Upload rejected (${error.code}). Please check the file and try again, or email ${PUBLIC_FORM_SUPPORT_EMAIL}.`;
        const clientCode = isFileSize ? upload_security_1.UPLOAD_TOO_LARGE_CODE : error.code;
        const attemptEvent = {
            attemptId: req.get('x-menumanager-attempt-id'),
        ...
        return res.status(statusCode).json({
            error: clientMessage,
            code: clientCode,
            ...(isFileSize ? { maxUploadBytes: upload_security_1.MAX_UPLOAD_BYTES } : {}),
        });
    }
