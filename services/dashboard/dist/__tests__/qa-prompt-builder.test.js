...
describe('buildFinalPrompt (extracted from handleBasicCheck)', () => {
    ...
    test('allergen key is appended when the base prompt lacks the heading', () => {
        const { prompt } = (0, qa_prompt_builder_1.buildFinalPrompt)('Minimal prompt.', {
    ...
    test('embedded set-menu section is appended when sections are detected', () => {
        const analysis = (0, embedded_set_menu_guard_1.analyzeEmbeddedSetMenus)([
            'FEAST MENU 65',
            'choice of',
            'STARTERS',
            'guacamole, chips',
    ...
    test('every emitted section id is documented in QA_PROMPT_SECTIONS', () => {
        const { sections } = (0, qa_prompt_builder_1.buildFinalPrompt)(BASE_PROMPT, {
            menuType: 'prix_fixe',
