...
describe('approved DOCX download fallback chain', () => {
    test('uses a local artifact without contacting SharePoint or regenerating', async () => {
        ...
        await expect((0, approved_download_1.resolveApprovedDownload)({ hasSharePointCopy: true, allowRegeneration: true }, deps))
    ...
    test('does not regenerate the original approved download', async () => {
        const deps = createDeps();
        await expect((0, approved_download_1.resolveApprovedDownload)({ hasSharePointCopy: true, allowRegeneration: false }, deps))
