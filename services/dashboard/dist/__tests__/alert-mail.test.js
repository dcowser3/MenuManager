...
describe('alert mail transport', () => {
    ...
    describe('buildGraphSendMailRequest', () => {
        test('builds an HTML message with base64 file attachments', () => {
            const payload = (0, alert_mail_1.buildGraphSendMailRequest)({ ...MESSAGE, cc: ['ops@example.com'] });
        ...
        test('writes directly into the recipient inbox when sendMail lacks Mail.Send', async () => {
            ...
            const fetchImpl = async (url, init) => {
                ...
                if (url.includes('/sendMail'))
                    return fakeResponse(403, { error: { code: 'ErrorAccessDenied' } });
                return fakeResponse(201, { id: 'msg-1' });
            };
            const result = await (0, alert_mail_1.sendAlertMail)(MESSAGE, { graphConfig: (0, alert_mail_1.buildGraphMailConfig)(GRAPH_ENV), fetchImpl });
