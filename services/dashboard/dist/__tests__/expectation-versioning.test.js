...
test('activation planner requires the exact selected rule to have been written first', () => {
    ...
    expect((0, expectation_versioning_1.planApprovedExpectationActivation)(envelope, [rule], [{ index: 4, ok: true, correctionId: 'r-a', location: 'Restaurant A', menuScope: 'food', isLocationSpecific: true }], [4])).toBeNull();
...
test('approved authority derives proposal envelope and activation preserves provenance', () => {
    const authority = (0, expectation_versioning_1.freezeExpectationEnvelope)({ policyVersion: 'p1', candidatePolicyVersion: 'p2', expectations: [
    ...
    expect(() => (0, expectation_versioning_1.validateApprovedExpectationAuthority)(authority)).not.toThrow();
    const derived = (0, expectation_versioning_1.deriveProposalBoundEnvelope)([{ change_type: 'terminology', original_text: 'house-made', corrected_text: 'housemade', is_location_specific: true, location: 'Restaurant A', applies_to_menu_type: 'food', expectation_activation: { source: 'model', ruleId: 'bad' } }], authority, 'cycle-1');
    ...
    const result = (0, expectation_versioning_1.planApprovedExpectationActivation)(derived.envelope, derived.rules, [{ index: 0, ok: true, correctionId: 'proposal-cycle-1-rule-0', location: 'Restaurant A', menuScope: 'food', isLocationSpecific: true }], [0]);
    ...
    expect((0, expectation_versioning_1.planApprovedExpectationActivation)(derived.envelope, derived.rules, [{ index: 0, ok: true, correctionId: 'proposal-db-999-rule-0', location: 'Restaurant A', menuScope: 'food', isLocationSpecific: true }], [0])).toBeNull();
    const bad = { ...authority, policyChangeApprovals: [{ ...authority.policyChangeApprovals[0], caseId: 'wrong' }] };
    expect(() => (0, expectation_versioning_1.validateApprovedExpectationAuthority)({ ...bad, sha256: 'tampered' })).toThrow();
});
