...
describe('learning submission display helpers', () => {
    test('prefers stored project name as the menu display label', () => {
        const decorated = (0, learning_submissions_1.formatLearningSubmissionDisplay)({
    ...
    test('falls back to useful filenames but ignores internal artifact names', () => {
        const internalOnly = (0, learning_submissions_1.formatLearningSubmissionDisplay)({
        ...
        const namedFile = (0, learning_submissions_1.formatLearningSubmissionDisplay)({
    ...
    test('decorates rows without failing when metadata lookup misses', async () => {
        const rows = await (0, learning_submissions_1.decorateLearningSubmissionsWithMenuNames)([
            { submission_id: 'sub-1' },
            { submission_id: 'sub-2' },
        ], async (submissionId) => {
    ...
    test('shortens UUID-style submission ids for table detail text', () => {
        expect((0, learning_submissions_1.shortSubmissionId)('83b0220a-ccf6-460c-be69-fa3d0a37bc22')).toBe('83b0220a');
        expect((0, learning_submissions_1.shortSubmissionId)('legacy-submission-id')).toBe('legacy-submi');
    });
});
