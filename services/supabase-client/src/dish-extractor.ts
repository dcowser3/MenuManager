...
const TRAILING_ALLERGEN_CLUSTER_REGEX = new RegExp(
    `(?:(?:\\s+|\\*)[(]?\\s*${ALLERGEN_CLUSTER_SEQUENCE}\\s*[)]?\\s*\\*?\\s*)+$`,
    'i'
);
...
export function isPrixFixeServicePeriod(servicePeriod: string | undefined): boolean {
...
export function normalizeDishPriceForStorage(
...
function isPotentialTwoLineDishName(nameLine: string, descriptionLine: string): boolean {
...
function isPotentialStandaloneDishName(line: string): boolean {
...
function isPotentialDescriptionLine(line: string): boolean {
...
function joinWrappedDishContinuation(line: string, nextLine?: string): { line: string; consumedLines: number } {
    ...
    if (
        !nextLineIsPriceOnly && (
            isFooterBoundary(cleanNextLine) ||
...
function isEventTitleLine(line: string): boolean {
    return /\b(?:feast|thanksgiving|christmas|valentine|new\s+year|story)\b/i.test(line.trim());
}

function isGeographicHeading(line: string): boolean {
    const normalized = line
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')
        .toLowerCase()
...
function isMenuTitleCategory(line: string): boolean {
    const cleanLine = line.trim();
    return (
        cleanLine.length > 5 &&
        cleanLine.length < 80 &&
...
function isShortCategoryHeading(line: string): boolean {
...
function isAllergenLegendPart(part: string): boolean {
...
function isNonDishLine(line: string): boolean {
    const normalized = line.replace(/\s+/g, ' ').trim();
    if (!normalized) {
        return true;
    }
...
function isSectionPriceLine(line: string): boolean {
...
function isPersistentSectionPriceLine(line: string): boolean {
    const normalized = line.replace(/\s+/g, ' ').trim();
...
function isPrixFixeMenuMarker(line: string): boolean {
    const normalized = line.replace(/\s+/g, ' ').trim();
...
function inferMenuPriceLabel(lines: string[]): string | undefined {
...
function extractSectionPrice(line: string): string | undefined {
...
function cleanSectionCategoryName(line: string): string {
    const cleaned = line
...
function cleanDescriptionText(description: string | undefined): string | undefined {
    const allergenClusterBeforePrice = new RegExp(
        `(?:\\s+|\\*)\\(?\\s*${ALLERGEN_CLUSTER_SEQUENCE}\\s*\\)?\\s*\\*?\\s+(?:[$€£]?\\d+(?:\\.\\d{1,2})?|\\d+\\s*cup\\s*/\\s*\\d+\\s*bowl)\\b.*$`,
        'i'
    );
    const allergenClusterBeforeNextDish = new RegExp(
        `(?:\\s+|\\*)\\s*${ALLERGEN_CLUSTER_SEQUENCE}\\s+[A-ZÀ-Þ].*$`,
        'i'
    );
...
function isReusableSectionPriceOnlyLine(line: string, category?: string): boolean {
    const price = extractPrice(line);
    if (!price) {
        return false;
    }
...
function isBeveragePriceCategory(category?: string): boolean {
...
function isLowercaseOneWordFragment(name: string, description?: string): boolean {
    return /^[a-z][a-z-]{2,}$/.test(name.trim()) && !!description?.trim();
}

function isLowercaseIngredientFragment(name: string, description?: string, price?: string): boolean {
...
function isShortLowercaseIngredientPhrase(name: string): boolean {
...
function isSameCategoryName(name: string, category?: string): boolean {
    if (!category) {
        return false;
    }
...
function dedupeExtractedDishes(dishes: ExtractedDish[]): ExtractedDish[] {
...
function removeTrailingPrice(line: string): string {
    return line
...
function isDateOrEventTimeLine(line: string): boolean {
    const normalized = line.replace(/\s+/g, ' ').trim();
...
function removeWrappingParens(line: string): string {
    return line.replace(/^\((.*)\)$/, '$1').trim();
}

function normalizeDishNameForCategory(name: string, category?: string): string {
...
function isSaladCategory(category: string): boolean {
    return /\b(?:salads?|ensaladas?)\b/i.test(category);
}

function uniqueAllergens(allergens: string[]): string[] {
    const unique: string[] = [];
...
function extractTrailingAllergens(line: string): { cleanedText: string; allergens: string[] } {
    ...
    const match = normalizedLine.match(
        new RegExp(`(?:^|[\\s([{])(${TRAILING_CODE_GROUP}(?:(?:(?:\\s*[,/.|]\\s*)|\\s+)${TRAILING_CODE_GROUP})*)\\s*$`, 'i')
    );
    if (!match) {
        return {
...
function normalizeAttachedAllergenCodes(line: string): string {
    return line.replace(
        new RegExp(
            `(?<![\\s,/|])([A-Za-zÀ-ÿ)])(${TRAILING_CODE_GROUP}(?:[,/|]${TRAILING_CODE_GROUP})+)(\\s*)$`,
            'i'
        ),
        '$1 $2$3'
    );
