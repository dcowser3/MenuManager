...
export const NULLABLE_COLUMNS: Record<string, string[]> = {
...
export interface TableSchema {
    columns: Set<string>;
    // NOT NULL columns without a DEFAULT — i.e. columns a client must supply on
    // insert. This is exactly the set PostgREST reports as `required`.
    requiredNoDefault: Set<string>;
}

export type DriftKind = 'missing_table' | 'missing_column' | 'unexpected_not_null';
...
export function parseSchemaSql(sql: string): Map<string, TableSchema> {
    ...
    while ((match = createRe.exec(sql)) !== null) {
        ...
        const requiredNoDefault = new Set<string>();
        for (const rawItem of splitTopLevel(body)) {
            const item = rawItem.trim();
            if (!item || CONSTRAINT_KEYWORDS.test(item)) continue;
            ...
            if (isNotNull && !hasDefault && !isSerial) requiredNoDefault.add(column);
        }
        if (columns.size > 0) tables.set(table, { columns, requiredNoDefault });
    }
    return tables;
}
...
export function parseOpenApiSpec(spec: any): Map<string, { columns: Set<string>; required: Set<string> }> {
    const out = new Map<string, { columns: Set<string>; required: Set<string> }>();
    ...
    for (const [table, def] of Object.entries<any>(defs)) {
        ...
        const required = Array.isArray(def?.required) ? def.required.filter((c: any) => typeof c === 'string') : [];
...
export function diffSchemas(
    expected: Map<string, TableSchema>,
    live: Map<string, { columns: Set<string>; required: Set<string> }>,
    ignoreTables: Set<string> = new Set(),
): DriftFinding[] {
    const findings: DriftFinding[] = [];
    for (const [table, expectedSchema] of expected) {
        if (ignoreTables.has(table)) continue;
...
export function diffNullableConstraints(
    live: Map<string, { columns: Set<string>; required: Set<string> }>,
    nullableColumns: Record<string, string[]> = NULLABLE_COLUMNS,
    ignoreTables: Set<string> = new Set(),
): DriftFinding[] {
    const findings: DriftFinding[] = [];
    for (const [table, mustBeNullable] of Object.entries(nullableColumns)) {
        if (ignoreTables.has(table)) continue;
        const liveSchema = live.get(table);
        if (!liveSchema) continue; // absence is diffSchemas' job
