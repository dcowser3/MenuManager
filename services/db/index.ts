...
import { detectNotNullDrift, NULLABLE_COLUMNS } from './lib/schema-drift';
...
async function findSubmissionByDisputeToken(token: string): Promise<any | null> {
    ...
    if (isSupabaseConfigured()) {
        const { data, error } = await getSupabaseClient()
            ...
            .maybeSingle();
        if (error) throw new Error(`Failed to look up dispute token: ${error.message}`);
        if (data) return data;
    }
    const submissions = JSON.parse(await fs.readFile(SUBMISSIONS_DB, 'utf-8'));
    return (Object.values(submissions) as any[]).find((row) => `${row?.approver_dispute_token || ''}`.trim() === normalized) || null;
}
...
async function findActiveDraftForMenu(menuId: string): Promise<any | null> {
    ...
    if (isSupabaseConfigured()) {
        const { data, error } = await getSupabaseClient()
            ...
            .order('updated_at', { ascending: false })
            .limit(1);
        if (error) throw new Error(`Failed to find active draft for menu: ${error.message}`);
        if (data?.[0]) return normalizeDraftSession(data[0]);
    }
    const drafts = await readLocalDraftSessions();
    const active = Object.values(drafts)
        .map(normalizeDraftSession)
        .filter((draft: any) => draft.menu_id === normalizedMenuId && draft.status === 'active')
        .sort((a: any, b: any) => Date.parse(b.updated_at || '') - Date.parse(a.updated_at || ''))[0];
    return active || null;
}
...
app.get('/submissions/dispute/:token', async (req, res) => {
    try {
        ...
        if (!submission) return res.status(404).json({ error: 'Dispute link not found' });
        return res.json({
...
async function fetchRequiredColumns(table: string): Promise<string[] | null> {
    ...
    try {
        ...
        if (!def || !Array.isArray(def.required)) return null;
        return def.required.filter((c: any): c is string => typeof c === 'string');
    } catch {
        return null;
    }
}
...
async function verifyCriticalSupabaseSchema(): Promise<void> {
    try {
        ...
        for (const [table, mustBeNullable] of Object.entries(NULLABLE_COLUMNS)) {
            const required = await fetchRequiredColumns(table);
            if (!required) continue; // spec unavailable — degrade to the runtime insert alert
            const drifted = detectNotNullDrift(required, mustBeNullable);
            if (drifted.length > 0) {
