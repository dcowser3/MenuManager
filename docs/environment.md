...
- `IMPROVE_MODEL` controls improvement-cycle analysis; its fallback chain is `IMPROVE_MODEL` → `PROMPT_REWRITE_MODEL` → `gpt-5.6-sol`.
...
| Variable | Description |
|----------|-------------|
| `TENANT_CONFIG_PATH` | Path to the white-label config bundle (branding, emails, allergen key, template markers, seed rules/properties). Absolute, or relative to the process CWD. Defaults to `<repoRoot>/config`. See [docs/onboarding-new-business.md](onboarding-new-business.md). |
| `SMTP_FROM` | From address used for Menu Manager notification emails. Recommended when `SMTP_AUTH=none`; must use an accepted sender domain for Microsoft 365 relay. Defaults to `GRAPH_MAILBOX_ADDRESS`, then `SMTP_USER`, then the config bundle's `emails.from`. |
...
| `AI_REVIEW_SEED` | Optional non-negative integer seed for production AI review calls, including Basic AI Check and post-submit review. Defaults to `42`; an explicitly empty value disables sending it. Seed reduces variance but does not guarantee determinism because backend batching can still vary. `BASIC_AI_CHECK_SEED` remains a temporary compatibility alias when `AI_REVIEW_SEED` is unset. |
...
| `CANONICAL_VOCABULARY_ENABLED` | Set `false` to disable approved-menu near-miss briefing and unresolved-spelling fallback suggestions (default: enabled). Reviewer-confirmed deterministic spelling/terminology rules still run. The vocabulary is built from active approved dishes plus full human-approved submission text and cached for 10 minutes. |
...
| `IMPROVE_MODEL` | OpenAI model for the improvement-cycle analysis call. Default chain: `IMPROVE_MODEL || PROMPT_REWRITE_MODEL || gpt-5.6-sol`. Proposal authoring is low-volume and quality-critical, so the recommended deployment value is the flagship `gpt-5.6-sol`; menu reviews remain independently configurable through `AI_REVIEW_MODEL`. Reasoning-shape detection (`isReasoningModel`) covers the o-series and GPT-5 family, so non-default `temperature` is omitted. The cycle request is ~30k+ tokens and fails fast when it exceeds the org's per-model TPM cap (see `isRequestTooLarge429`). |
...
| `IMPROVE_MAX_RETRIES` | C1: when an automated prompt-shape guard (broken code-fence structure or echoed input context) discards the model's rewrite, the cycle re-calls the LLM with a corrective addendum this many times before giving up (default: `2`, i.e. up to 3 attempts total). The sentinel `UNCHANGED` is a deliberate no-change and is never retried. On exhaustion the proposal's disposition is `no_change_guard_discarded`. |
| `IMPROVE_RATE_LIMIT_RETRIES` | How many times the improvement LLM call backs off and retries on an OpenAI `429` before failing the cycle (default: `6`). It honors the `Retry-After` header or the "try again in Xs" hint (padded 1s, capped 90s). Needed because C1's guard-retry can fire a second large `o3` call within the same minute; the default `o3` per-minute token cap (30k TPM) is easily exceeded by two ~26k-token calls, so the second must wait for the window to clear rather than aborting the run. |
| `IMPROVE_TRANSIENT_RETRIES` | How many times the improvement LLM call retries a **non-429** transient failure — HTTP 5xx or a network fault (`fetch failed`, `ECONNRESET`, `socket hang up`, …) — before failing the cycle (default: `4`, backoff 5s/15s/45s/90s). Separate budget from `IMPROVE_RATE_LIMIT_RETRIES`. Added after a single OpenAI `500` aborted the Jul 25 2026 run outright. |
| `IMPROVE_REGRESSION_ATTRIBUTION` | Targeted failure attribution for combined prompt + replacement-rule proposals (default: enabled). After the normal eval confirms regressions, the cycle re-runs only those failed case IDs as prompt-only and rules-only variants and stores the per-case cause on the proposal. Set `0` to disable the additional targeted API calls. |
| `IMPROVE_MIN_HOURS_BETWEEN_PROPOSALS` | Cadence gate (default: `40`). The cron now runs **nightly**; the cycle exits quietly if the last proposal it produced is newer than this. Keeps the reviewer-facing every-other-day rhythm while letting a night that crashed retry itself the next night instead of waiting out a 48h cron gap. Set `0` to disable (propose every night); `--force` / `--consolidate` bypass it. |
...
Dashboard alert email (system alerts, form-failure alerts, user problem reports) prefers **Microsoft Graph `sendMail`** over HTTPS when `GRAPH_TENANT_ID`/`GRAPH_CLIENT_ID`/`GRAPH_CLIENT_SECRET` plus a sender mailbox (`GRAPH_MAILBOX_ADDRESS` or `GRAPH_USER_EMAIL`) are configured, falling back to SMTP otherwise. Submission confirmation emails reuse the same transport, but drop reserved placeholder recipients such as `example.com`, `example.net`, `example.org`, `.test`, `.invalid`, `.localhost`, and `.example` before sending so demo form values do not create Microsoft 365 delivery failures. This matters in production: Lightsail blocks outbound port 25 by default, so the Microsoft 365 IP-relay SMTP path times out. Graph requires the `Mail.Send` application permission with admin consent on the app registration. Until that consent is granted, a 403 from `sendMail` automatically falls back to writing the alert directly into the in-tenant recipient's inbox via the already-granted `Mail.ReadWrite` permission (see [design-docs/user-error-reports.md](design-docs/user-error-reports.md)). If token acquisition fails with `AADSTS7000215` / `invalid_client`, Azure rejected the configured client secret; create a new client secret, copy its one-time **Value** (not the Secret ID), update `GRAPH_CLIENT_SECRET` and `GRAPH_CLIENT_SECRET_EXPIRES`, then restart the dashboard.
