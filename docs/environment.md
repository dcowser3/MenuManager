...
| `IMPROVE_MODEL` | OpenAI model for the improvement-cycle analysis call. Default chain: `IMPROVE_MODEL || PROMPT_REWRITE_MODEL || o3` (reasoning-class). Reasoning-shape detection (`isReasoningModel`) covers the o-series and the gpt-5 family (gpt-5, gpt-5.1, gpt-5-mini, …) — gpt-5 models reject non-default `temperature`, so they must take the reasoning path. Production uses `gpt-5.1` (500k TPM): the cycle's request is ~30k+ tokens, and a single request larger than the org's per-model TPM cap fails fast with no retry (see `isRequestTooLarge429`) — o3's default 30k TPM tier is too small. |
