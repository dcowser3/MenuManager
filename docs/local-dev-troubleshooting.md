...
Internal service clients default to a 5-second timeout (`INTERNAL_API_TIMEOUT_MS`) unless a route sets a more specific timeout. Basic AI Check overrides this with `BASIC_AI_CHECK_TIMEOUT_MS` (default `120000`) because it runs as a background job instead of a long gateway-facing request, post-submit full AI review uses `AI_REVIEW_SUBMIT_TIMEOUT_MS`, ClickUp task creation uses `CLICKUP_TASK_CREATE_TIMEOUT_MS` (default `60000`), and browser approval editor finalization uses `CLICKUP_APPROVAL_FINALIZE_TIMEOUT_MS` (defaulting to the task-create timeout) because ClickUp API calls plus attachment uploads, status moves, and assignments can exceed 5 seconds. The public form starts Basic AI Check with `/api/form/basic-check/start` and polls `/api/form/basic-check/status/:checkId`; submit remains blocked while the job is pending. If a dashboard route waits indefinitely, first confirm Docker Desktop is responding with `docker version`; a half-stuck Docker engine can hold ports open while no service process can answer.
...
- The upload-first form confirms delivery of `submit_started` and `submit_client_exception` telemetry with an HTTP response instead of trusting `sendBeacon` queueing. Each event includes a browser timestamp, phase (`before_fetch`, `fetch_submit`, `read_response`, etc.), online/visibility state, elapsed time, and request/HTML lengths. If telemetry itself cannot reach the server, the browser retains at most 12 metadata-only events and retries them when connectivity returns or the form opens again. Delayed rows have `details.delayedDelivery=true`; use `details.clientTime` rather than row `created_at` to place them on the incident timeline. Browser storage excludes menu text, contact details, and AI suggestions. No telemetry can identify a network intermediary's policy reason; inspect the browser network trace or intermediary logs when nginx has no matching request.
...
- **Shared workspace packages in Docker:** every shared package imported by a service, including `diff-core`, `internal-auth`, `llm-adapter`, `supabase-client`, and `tenant-config`, must be included in `docker/Dockerfile.dev` before `npm ci` and compiled into the image; otherwise dev containers may fail to resolve `@menumanager/*` imports until `./dev-up.sh --rebuild`.
