...
Before approval, Isabella can open `/reviews` in the dashboard service to see submissions whose Menu Manager DB status is `pending_human_review` or `submitted_no_ai_review`; `/dashboard` and `/review-queue` redirect to the same queue. Each row links to `/approval/:submissionId`, `/review/:submissionId`, and the original DOCX download. The queue is DB-status driven, not a live ClickUp query: browser approval finalization or the ClickUp webhook/backfill must update a submission to `approved` before it disappears. Isabella-submitted direct handoffs with a linked ClickUp task are excluded because they bypass Isabella review and go straight to Marketing.
