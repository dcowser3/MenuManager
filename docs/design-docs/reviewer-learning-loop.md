...
- Displays auto-scanned detected patterns with confidence, evidence status, and implementation coverage. A threshold-met pattern is labeled as a candidate/reference signal on the dashboard, not as an active runtime replacement; each detected pattern also says whether it is covered by built-in Pre-AI code, an accepted exact replacement, a code guard, a pending/rejected correction rule, context guidance only, or no implementation yet.
- Displays pending `correction_rules` for accept/reject review and accepted rules that can feed deterministic pre-AI checks. The "Accepted Exact Replacement Rules Active In Pre-AI" section only includes accepted before/after replacements that pass the same safety eligibility helper used by the runtime Pre-AI pass.
...
- Categorizes every accepted explanation by implementation lane in the audit log: active exact replacement, active code guard, context guidance only, manual guidance, or manual review only. The same row also states what the code now does, or why no code path applies it.
