...
- The upload review path and Approved Menus `Edit This Menu` drafts converge on the same `reviewedContentArea` editor. A shared rich-text controller supplies the Bold action, preserves/restores the selected DOM range before the toolbar takes focus (required for reliable Safari/WebKit behavior), and emits the normal editor input event so preview and draft autosave stay synchronized. Browser-specific handling lives only in `public/js/rich-text-editor.js`.
