...
- The upload review path and Approved Menus `Edit This Menu` drafts converge on the same `reviewedContentArea` editor. A shared rich-text controller supplies both an unobtrusive in-editor Bold action and Command-B, preserves/restores the selected DOM range before formatting (required for reliable Safari/WebKit behavior), and emits the normal editor input event so preview and draft autosave stay synchronized. Browser-specific handling lives only in `public/js/rich-text-editor.js`.
...
- When an unapproved/redlined DOCX is used as the approval source, the editor keeps separate canonical text baselines: original text is reconstructed from deleted/unmarked DOCX runs, and fixed text is reconstructed from inserted/unmarked DOCX runs. The left rich editor starts from the fixed text; the right preview renders `original -> current editor text` on every update.
- Extractor-provided paragraph annotation ranges are used only to derive those canonical original/fixed sides from the imported preview text. The live approval preview no longer reinserts old deletion runs into the revised text, so blank-line or word edits in the left editor cannot shift old spans into repeated nearby menu text.
...
- If the reviewer changes accepted text back to the original deleted value, the `original -> current` comparison naturally produces no insertion/deletion for that group instead of stacking stale imported markup.
- Whole-row imported deletions and insertions remain separate because the shared renderer performs row-level alignment before token-level diffing.
- The token renderer filters whitespace-only anchors inside changed runs and renders replacement runs as grouped deletion/insertion chunks. This prevents adjacent replacement spans such as deleted `Wagyu` plus inserted `Mignon` from becoming synthetic tokens or duplicating old deletion text when a reviewer removes one accepted word.
- Rows with no word-like tokens, such as dashed separators, are only aligned when their normalized literal text matches. They are not protected from diffing; this prevents separator rows from acting as wildcard anchors while still highlighting them if the separator text actually changes.
...
- The approval preview worker receives the same canonical original text and original HTML as the main-thread fallback, then renders against the current editor text. This keeps the worker path deterministic without a separate imported-redline resolver.
