...
- The live preview carries imported annotation boundaries into the revised preview text before token diffing. This prevents adjacent replacement spans such as deleted `Wagyu` plus inserted `Mignon` from becoming synthetic tokens and duplicating old deletion text when a reviewer removes one accepted word.
