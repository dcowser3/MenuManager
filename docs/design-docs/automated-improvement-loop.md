...
1. **Lock + gate**: `tmp/improvement-cycle/.lock` (stale after 6h) plus one-proposal-per-day (`cycle_id` = `YYYY-MM-DD`). Exits quietly when unconsumed corrections < `IMPROVE_MIN_NEW_CORRECTIONS` (default 1). When a previous proposal is still pending, the daily run skips new proposal generation but sends a reminder email for the pending proposal so the queue does not stall silently. `--force`, `--skip-eval`, `--dry-run` flags.
