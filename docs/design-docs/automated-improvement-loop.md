...
- **Config**: `--prompt <file>`, `--model` (default `REVIEW_EVAL_MODEL || AI_REVIEW_MODEL || gpt-4o-mini`), `--rules live|snapshot:<f>|candidate:<f>` (candidate = live accepted + proposed file), `--no-deterministic`, `--no-ai` (echo feedback; deterministic-only), `--limit`, `--case <id>`. Proposal-shaped candidate rules receive transient `accepted` status and synthetic eval-only ids so they execute exactly as they would after approval without mutating the stored proposal.
...
1. **Lock + gate**: `tmp/improvement-cycle/.lock` (stale after 6h) plus one-proposal-per-day (`cycle_id` = `YYYY-MM-DD`). Exits quietly when unconsumed corrections < `IMPROVE_MIN_NEW_CORRECTIONS` (default 1). When a previous proposal is still pending **and there are no new unconsumed corrections and the live review baseline is unchanged**, the daily run skips new proposal generation but sends a reminder email for the pending proposal so the queue does not stall silently. The baseline fingerprint covers the effective prompt, executor model, deterministic code-rules manifest, and accepted database rules. New corrections or a changed/missing baseline fingerprint make the cycle **supersede** the old pending proposal: it generates a fresh proposal from the current approved effective prompt using the pending proposal's unresolved source corrections plus any new ones, marks the old proposal `status: superseded`, and emails the new proposal instead of a reminder. Manual `--force` (or the dashboard on-demand button) also supersedes a pending proposal even with zero new corrections. Before prompt generation, replay-proven `now_correct` rows are removed from LLM context and later retired under a durable resolution marker, preventing already-implemented explanations from being proposed again. `--skip-eval`, `--dry-run` flags.
2. **Effective prompt**: latest approved proposal from `prompt_proposals` beats `qa_prompt.txt` (the file is baked into the Docker image, so dashboard-approved prompts would be reverted on redeploy — the dashboard now also restores it at startup via `syncEffectivePromptFromDb()`). Repository prompt changes must be promoted deliberately with `commit-runtime-prompt.js`; production promotion supports `--expected-db-sha256` so a stale deployment cannot overwrite a newer human-approved prompt.
...
5. **Auto-eval**: spawns the eval harness twice — baseline (current prompt + live rules) and candidate (proposed prompt + `candidate:` rules) — and stores `eval_summary` + `eval_status` (`passed`/`regressed`/`failed`/`skipped`). Regressed proposals are stored and flagged, never dropped. When both the prompt and replacement rules changed, confirmed regression case IDs are then replayed through targeted prompt-only and rules-only variants (enabled by default; `IMPROVE_REGRESSION_ATTRIBUTION=0` disables it). Each failure is labeled `prompt`, `rules`, `both`, or `interaction_or_unstable` on the proposal page without paying for two more full-dataset runs. Eval also supplies the full set of human-approved ground truths to the contextual vocabulary layer and bypasses its production TTL cache, so baseline and candidate cannot accidentally share one side's rules or vocabulary.
6. **Store + notify**: inserts the `prompt_proposals` row (`source: improvement_cycle`), marks unresolved corrections consumed under the proposal `cycle_id`, writes artifacts to `tmp/improvement-cycle/<cycle>/`, and emails `IMPROVE_NOTIFY_EMAIL || FORM_ATTEMPT_ALERT_EMAIL` via the Graph alert-mail transport with a public link to `/learning/prompt-proposal`. Corrections whose full-line replay is `now_correct` receive a durable `resolved-by-current-pipeline:<cycle>` marker instead; rejection only releases unresolved proposal-stamped corrections, so deployed fixes do not recycle forever. The link base is `DASHBOARD_PUBLIC_URL` when set, otherwise `DASHBOARD_URL`, otherwise local development falls back to `http://localhost:3005`.

**Dashboard** (`/learning/prompt-proposal`): the proposal page shows the eval verdict (baseline vs candidate composites, improved/same/regressed, per-case regression table), per-case prompt/rules attribution when a combined candidate regresses, the proposed replacement rules as a checked-by-default checkbox list, validation notes from dropped/guarded LLM output, and code recommendations. Trigger rows label their delta as the **confirmed** delta because it may come from a fresh back-to-back re-run rather than the raw baseline/candidate percentages shown beside it. The top-line correction count is labeled as source corrections because it counts input evidence sent to the cycle, not the smaller set of deterministic rules that survived validation. On approve, the review route writes the prompt (as before), records `accepted_rules`, and inserts each checked rule into `correction_rules` (`status: accepted`, `source: system`, `submission_id: proposal-<id>`) so the pre-AI deterministic pass applies them immediately. The `/learning` Pending Rules table shows each pending rule's creation time and origin (`General add-rule area`, `Menu correction page` with menu name, or detected-pattern scan) and excludes source correction rows once the cycle has stamped `prompt_cycle_id` or `consumed_at`; those rows are evidence consumed by the proposal, not separate manual-review tasks. `npm run prompt:rewrite` remains as a manual fallback until the cycle has run in production.
...
**Reviewer attribution is mandatory.** Both human save paths — the `/learning` General add-rule area and the `/learning/submission/:id` Menu correction page — require a Reviewer Name. The submission page collects it once for individual or bulk explanation saves. `buildCorrectionRuleRecord` throws `reviewer_name is required` for any non-`system` source (both forms also validate client-side), so a human rule can never land unattributed. Only `source: 'system'` rows (cycle approvals, detected-pattern scans) are exempt. Rows created before this was enforced have `reviewer_name = null` and cannot be back-attributed — there is no per-user auth on the dashboard, so the reviewer field is the only attribution captured.
...
## August 28, 2026 safe rerun hardening

- **Rules-only fallback:** when a combined prompt+rules candidate regresses, targeted attribution still identifies the failing surface. If every confirmed combined regression is prompt-caused, no correction relies solely on the discarded prompt edit, and a new uncapped full-suite run of the current prompt plus candidate rules has zero confirmed regressions, the cycle automatically discards the prompt rewrite and stores the safe rules-only proposal. The original rewrite remains in `discarded_regressed_prompt.txt`; `eval_summary.rules_only_fallback` records why it was replaced and the proposal page shows the fallback explicitly. A configured `IMPROVE_EVAL_LIMIT` never weakens this gate: the fallback builds an uncapped baseline and candidate pair.
- **Candidate-rule activation proof:** eval reports record full-suite pre-AI and post-AI activation counts plus case ids for every proposed rule. The improvement cycle also applies each candidate rule directly to the motivating unresolved replay text and records targeted-replay activation counts and correction ids. This covers safe rules for rare intermediate forms that the historical end-to-end suite does not reproduce. A rules-only proposal (including a salvaged fallback) passes only when every candidate rule fires in the full suite or targeted replay and the suite has no confirmed regressions. Trigger improvement cannot override incomplete activation evidence, and the approval gate independently rechecks the evidence even if a stored status says `passed`. Missing or inactive evidence produces `no_effect` and blocks approval; the proposal page shows all three activation sources.
- **Partial compound replay:** replay evidence now distinguishes `partially_correct` from `still_missed`. It records expected token changes already applied and those still absent, so a compound reviewer row cannot falsely prove that an existing atomic rule failed merely because the same row also asks for unsupported additions. The proposal model must route only the remaining delta and may dismiss an addition only when it cannot be inferred safely from the source or reviewer explanation.
