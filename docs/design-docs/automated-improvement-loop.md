...
- **Candidate-rule activation proof:** eval reports record pre-AI and post-AI activation counts plus case ids for every proposed rule. A rules-only proposal (including a salvaged fallback) passes only when every candidate rule fires at least once and the suite has no confirmed regressions. Missing or inactive evidence produces `no_effect` and blocks approval; the proposal page shows the per-rule evidence table.
