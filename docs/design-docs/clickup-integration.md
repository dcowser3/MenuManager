...
- If the task or attachment handoff fails, the submission remains `sent_to_marketing` and retains its ClickUp task link for retry. The authenticated `POST /webhook/backfill-isabella-direct` route repairs legacy direct rows only when the current ClickUp task is still in `To Do`, has a DOCX attachment, and the local DOCX source exists; reruns are idempotent because repaired rows no longer match the legacy status.
