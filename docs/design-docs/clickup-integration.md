...
- If the submitter email is the configured `emails.clickupHandoffSubmitter` (Isabella in the default tenant), creates the task directly in `CLICKUP_POST_APPROVAL_STATUS` (`To Do` by default) and assigns the resolved Marketing users instead of the Isabella assignee. This direct handoff does not use `CLICKUP_CORRECTIONS_STATUS`, and the webhook treats ClickUp's `Approved` status as passive/manual, so Isabella submissions are not placed in or pulled back from the `Approved` column by automation.
- After the direct handoff's generated DOCX is successfully attached to the ClickUp task in `To Do`, the DB submission is finalized as `approved`, its generated DOCX is recorded as an `approved_docx` asset, and the existing DB approval hook resolves/advances the menu entity pointer. This makes the menu visible on Approved Menus immediately while preserving ClickUp `Approved` as a passive design-stage label.
- If the task or attachment handoff fails, the submission remains `sent_to_marketing` and retains its ClickUp task link for retry. The authenticated `POST /webhook/backfill-isabella-direct` route repairs legacy direct rows in `sent_to_marketing`, `pending_human_review`, or `submitted_no_ai_review` only when the submitter is the configured Isabella handoff identity, a ClickUp task is linked, the current task is still in `To Do`, a DOCX attachment exists, and the local DOCX source exists; reruns are idempotent because repaired rows no longer match those candidate statuses. The DB update boundary also prevents an asynchronous AI write from moving a linked direct handoff back into review after it is sent or approved.
...
- **Operational repair:** `POST /webhook/backfill-isabella-direct` (internal-auth protected; never treats ClickUp `Approved` as an eligibility status)
