...
- accepted `correction_rules` bounded replacements whose status is `accepted` and whose property and menu-type scopes match the submitted property/template type; spelling/diacritic learned rules with tone marks match accent-insensitively, while terminology/grammar/punctuation rules remain exact

Learned rules are intentionally constrained to bounded spelling, diacritic, terminology, grammar, and punctuation replacements. Broad `content` rules remain reviewer/prompt material because they can change meaning. The deterministic pass handles the `aji`/`ahi` distinction explicitly: Spanish `aji` is corrected to `ají`, while Hawaiian `ahi tuna` phrase variants such as `ají tuna`, `ahí tuna`, and `aji tuna` are corrected to `ahi tuna` before the general `aji` diacritic rule runs. If accepted rules cannot be loaded, the dashboard fails open and still runs the built-in deterministic checks plus AI review.
