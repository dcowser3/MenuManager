...
- Stale suggestions describing a change that is already present in the corrected menu are dropped by `applyHighConfidenceSuggestionsToMenu` regardless of type or confidence, so the UI never shows an `Apply Change` button that cannot succeed. This includes the case where the model applied the fix itself and the suggestion's `menuItem` carries the old (e.g. misspelled) text that no longer matches any corrected line — line matching retries with the `from`→`to` substitution applied to the menu item.
