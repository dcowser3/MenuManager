...
- Frozen artifacts carry a deterministic SHA-256 and reject post-freeze tampering or more than 5,000 generated tests. The hash recursively sorts object keys while preserving array order, so JSONB-style key reordering on persistence/readback cannot change artifact identity.
...
The focused current-branch suite has 8 tests covering authority, four-stage known/unknown handling, browser-repair evidence, menu-update exclusion, scoped families and semantic abstention, canonical scope resolution, deterministic execution, JSONB-style key-reordered readback, tamper rejection, and the test-count bound. Dashboard TypeScript compilation and emitted source/test parity are required for this slice.
