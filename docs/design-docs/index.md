...
| [Menu as an Entity + Lightweight Identity](menu-entity-and-identity.md) | Phases 1–3 built (schema/backfill + write-path pointer moves + menu-centric read path) | First-class `menus` records with a current-version pointer (replaces inferred supersede/staleness), menu-centric Approved Menus page, staged identity: remembered profile → approver dispute link ("if you did NOT approve this") → (deferred) sign-in |
