...
| [Menu as an Entity + Lightweight Identity](menu-entity-and-identity.md) | Phases 1–4 built (menu entity end-to-end + remembered profile); Stage-2 dispute link pending | First-class `menus` records with a current-version pointer (replaces inferred supersede/staleness), menu-centric Approved Menus page, staged identity: remembered profile → approver dispute link ("if you did NOT approve this") → (deferred) sign-in |
