...
| [Review-learning B5-C2b draft broker](review-learning-b5-c2b-draft-broker.md) | Implemented, callable offline boundary | Explicit code-candidate authorization, hash-bound bounded request/reservation, ambiguous-charge handling, and C2a-only draft handoff; independent verification remains deferred |
| [Review-learning B5-C2c1 proof runner](review-learning-b5-c2c1-proof-runner.md) | Implemented, credential-free boundary | Independent proof runner plus fixed B5-C2c2 Docker/unit boundary (JS fixed-run contract; TS/Jest blocked pending image transformer); replay and delivery remain explicitly blocked until repository-owned drivers exist; no provider, live DB, approval, deployment, or activation |
