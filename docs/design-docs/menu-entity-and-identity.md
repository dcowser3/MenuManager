...
**Fixes (2026-07-17, post-backfill regressions):** (a) active drafts didn't show on cards — the Phase 3 rekey backfilled `draft_sessions.menu_id` while `menus` was still empty, so pre-existing active drafts had `menu_id = null`. `scripts/backfill-menus.js` gains `--relink-drafts` (re-derive each active draft's `menu_id` from its baseline in both stores), and `--apply`/`--apply-review` now run it automatically at the end so the ordering can't bite again. (b) The `menu-actions` column is restored to uniform full-width buttons (badge → actions → downloads). (c) A draft whose baseline is no longer the menu's current version now shows a "Draft started from an older version" badge on the card (`activeDraft.staleBaseline`), matching the form's at-open warning.

**Fix (2026-07-17):** `menus.current_submission_id` was created as `UUID`, but the pointer holds a *public* submission id (`getPublicSubmissionId` = `legacy_id || id`); `clickup_history_import` rows carry non-UUID `legacy_id` strings, which broke `--apply` and was silently failing the Phase 2 pointer move for legacy rows. Migration `20260717_menu_current_submission_id_text.sql` moves the column to `VARCHAR(100)` (`submissions.menu_id` / `draft_sessions.menu_id` stay UUID — they hold minted menu ids). The backfill's Supabase link filter is now UUID-safe (matches `id` or `legacy_id` by shape), `--apply` is idempotent (reuses the menu a group is already linked to; overwrites the local mirror to the applied set so a partial run can't leave local and Supabase disagreeing), and a `--reset` flag clears `menus` + nulls `submissions.menu_id` in both stores for a clean rerun.

**Phase 5 built (2026-07-16, approver dispute link — independent):** negative confirmation. Each submission mints a per-submission `approver_dispute_token` at creation (migration `20260716_approver_dispute.sql` adds `approver_dispute_token` / `approver_disputed_at` / `approver_dispute_note`). The post-submit confirmation email is now role-segregated: the submitter receipt is unchanged, and the **approver** copy carries one line — "If you did **not** approve this menu, let us know" → the public, rate-limited `GET /approval-dispute/:token`. The route renders a tenant-branded confirm page (button + optional note, no auth); `POST` records `approver_disputed_at` + note (idempotent — re-visits show the recorded state), fires a reviewer notification (`logAlert` + email to `internalReviewer`, fire-and-forget), and thanks the approver. The review detail page shows a prominent dispute banner. **No automatic unwind** (invariant 6); attestation still gates submit. Token validity is indefinite in v1. This completes Identity Stages 1–2; Stage 3 (real sign-in) remains deferred.

**Phase 4 built (2026-07-16, remembered profile — independent; resume attribution corrected 2026-09-02):** the submission form and design-approval page store `{ name, email, jobTitle }` in `localStorage` (`menumanager.submitterProfile.v1`) on successful submit and on submitter-autocomplete selection, and prefill the Submitter Information fields on ordinary opens. A previously saved shared draft is the exception: its saved submitter values are cleared and browser-profile prefill is suppressed so the current person must provide the identity recorded on the final submission. Draft autosave's `last_edited_by` may still fall back to the stored profile name as a display-only editing hint. No accounts, no access control — client-side convenience only (Stage 1). Stages 2–3 (approver dispute link, sign-in) still pending.

**Phase 3 built (2026-07-16, read path):** the Approved Menus page is menu-centric — one card per menu (via `listMenuCards`/`groupApprovedIntoMenuCards`), current-version pointer replaces lineage-children inference, version-history expander with view/download-only old versions. Draft invariant re-keyed to `menu_id`: `POST /draft-sessions` rejects starting from an outdated version (409 `currentVersion`) and enforces one active draft per menu (migration `20260716_menu_entity_draft_rekey.sql` swaps the partial unique index; a per-base guard remains for the un-backfilled tail). The route tolerates a not-yet-migrated `menus` table (falls back to per-submission cards). Requires the Phase 1 migration applied; fully menu-centric after the backfill runs. The `form.ejs` freshness heuristic remains as a soft pre-submit hint, now backstopped by the authoritative server-side pointer gate. Deferred cleanup: the `enrichApprovedMenuList` dual-id join is now off the hot path but not yet deleted.
...
3. **Read path:** menu-centric Approved Menus page; draft invariant re-keyed to `menu_id`; retire heuristic gating. ✅ Built 2026-07-16.
4. **Identity Stage 1** (can ship independently, any time): localStorage profile prefill + `last_edited_by` stamping. ✅ Built 2026-07-16.
5. **Identity Stage 2:** approver dispute link ("If you did NOT approve this menu…") in the existing approver email. ✅ Built 2026-07-16.
