...
**Phase 4 built (2026-07-16, remembered profile — independent):** the submission form and design-approval page store `{ name, email, jobTitle }` in `localStorage` (`menumanager.submitterProfile.v1`) on successful submit and on submitter-autocomplete selection, and prefill the Submitter Information fields on open. Prefill runs **after** draft restore and only fills fields still empty — draft-saved `form_state` (the `5181d13` persistence) always wins. Draft autosave's `last_edited_by` falls back to the stored profile name when the visible field is empty. No accounts, no access control — client-side convenience only (Stage 1). Stages 2–3 (approver dispute link, sign-in) still pending.
