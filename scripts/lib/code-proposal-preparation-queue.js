...
async function preparePendingCodeProposalQueue(options = {}) {
    ...
    for (const proposal of proposals) {
        try {
            results.push(await prepareCodeProposalQueue({ ...options, proposal, manualExclusionArtifact: options.manualExclusionArtifacts?.[proposal.id], query: snapshot.query, pendingEnumeration: { complete: true, pages: snapshot.enumeration.pages, cutoff: snapshot.enumeration.cutoff, rows_count: snapshot.rows.length, row_ids: snapshot.rows.map((row) => row.id), query: snapshot.query, global_snapshot_sha256: snapshot.snapshot_sha256 }, enumeration: { ...snapshot.enumeration, global_snapshot_sha256: snapshot.snapshot_sha256 } }));
        } catch (error) {
