...
function parseWorkerOutput(stdout, stderr, spec = null) {
    ...
    if (spec) {
        ...
        const allowed = new Set(['protocol_version', 'status', 'phase', 'arm', 'seed', 'run_id', 'runtime_id', 'image_id', 'error', 'blocked', 'exit_code', 'report', 'report_id', 'output', 'response', 'diagnostics', 'outcomes', 'driver', 'source_manifest_sha256', 'baseline_submitted_text', 'candidate_submitted_text', 'baseline_submitted_html', 'candidate_submitted_html', 'baseline_submitted_html_text', 'candidate_submitted_html_text', 'baseline_source_hashes', 'candidate_source_hashes', 'baseline_browser_version', 'candidate_browser_version', 'quill_version']);
...
function createDockerC2c2Executors(options = {}) {
    ...
    return Object.freeze({
        ...
        deliveryExecutor: (input) => runDockerInvocation(buildDockerInvocation({ ...frozen, phase: 'delivery', arm: 'paired', seed: input.seed, runId: input.runId, baselineRoot: frozen.baselineRoot, candidateRoot: frozen.candidateRoot, testBundleRoot: input.plan.paths.testBundle, outputRoot: frozen.outputRoot, request: { phase: 'delivery', arm: 'paired', seed: input.seed, run_id: input.runId, delivery_fixture: input.delivery_fixture, correction: input.correction, plan: input.plan } }), { timeoutMs: frozen.timeoutMs, spawn: frozen.spawn, inspectImage: frozen.inspectImage }),
    });
}
