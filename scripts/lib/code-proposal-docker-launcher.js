...
function buildDockerInvocation(options = {}) {
    ...
    const args = ['run', '--rm', '--name', name, '--entrypoint', FIXED_ENTRYPOINT, '--label', `com.menumanager.c2c2.owner=${attemptId}`, '--label', `com.menumanager.c2c2.name=${name}`, '--network', 'none', '--cap-drop', 'ALL'];
    if (phase !== 'delivery') args.push('--cap-add', 'SETUID', '--cap-add', 'SETGID');
    args.push('--security-opt', 'no-new-privileges:true', '--read-only', '--pids-limit', '128', '--memory', '1g', '--cpus', '1', '--user', phase === 'delivery' ? '65532:65532' : '0:0', '--tmpfs', '/tmp:rw,noexec,nosuid,size=64m', '--tmpfs', '/runner/output:rw,noexec,nosuid,size=64m,mode=1777');
...
function parseWorkerOutput(stdout, stderr, spec = null) {
    ...
    if (spec) {
        ...
        const allowed = new Set(['protocol_version', 'status', 'phase', 'arm', 'seed', 'run_id', 'runtime_id', 'image_id', 'error', 'blocked', 'exit_code', 'report', 'report_id', 'output', 'response', 'diagnostics', 'outcomes', 'driver', 'driver_sha256', 'controls', 'chromium_sandbox_enabled', 'isolation_boundary', 'delivery_claim', 'reviewed_state_selection', 'full_form_assembly', 'source_manifest_sha256', 'delivery_fixture_sha256', 'baseline_submitted_text', 'candidate_submitted_text', 'baseline_submitted_html', 'candidate_submitted_html', 'baseline_submitted_html_text', 'candidate_submitted_html_text', 'baseline_source_hashes', 'candidate_source_hashes', 'baseline_browser_version', 'candidate_browser_version', 'quill_version']);
