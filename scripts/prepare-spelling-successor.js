...
function evaluateRules(currentRules, rules, routes) {
    ...
    const comparisons = cases.map(([input, expected], index) => ({ case_id: `spelling-${index}`, input, expected, baseline: baselineOutputs[index], candidate: candidateOutputs[index], improved: baselineOutputs[index] !== expected && candidateOutputs[index] === expected, regressed: candidateOutputs[index] !== expected }));
    const regressions = comparisons.filter((comparison) => comparison.regressed);
    const summary = { baseline: { label: 'baseline', casesEvaluated: cases.length, deterministicOutputs: baselineOutputs }, candidate: { label: 'candidate', casesEvaluated: cases.length, deterministicOutputs: candidateOutputs }, comparedCases: comparisons.length, regressed: regressions.length, regressions, candidate_rule_activations: candidateRuleActivations, triggers_improved: comparisons.filter((comparison) => comparison.improved).length, triggers_regressed: regressions.length, triggers_unchanged: comparisons.filter((comparison) => !comparison.improved && !comparison.regressed).length };
